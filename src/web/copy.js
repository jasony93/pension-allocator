...
// D61(관리자 판정, 소유자 지시, 세 번째 같은 방향) — 「법령이 정한 것」·「이
// 계산기가 정한 것」 태그(옛 `FILL_ORDER_TAG_FACT`·`FILL_ORDER_TAG_PRODUCT`)를
// **쌍으로** 지웠다. 하나만 지우면 남은 쪽이 짝을 잃어 "이 계산기가 정했다"만
// 말하고 무엇이 법인지는 안 말하게 되어, 지우려는 방향과 반대로 무거워진다.
// **태그가 붙어 있던 문장(아래 두 함수)은 남긴다** — 소유자가 초기 회차에 물은
// "왜 IRP를 연금저축보다 먼저 채우나"에 답하는 것은 설명이지 배지가 아니다.
// 태그에 붙어 있던 근거 조항 칩(`LawChip`)도 태그와 함께 나갔다(`result-panel.js`
// `fillOrderNote`) — 배제 사유(`eligibility.js`)의 `LawChip`은 소유자가
// 지목하지 않았으므로 그대로 둔다. 지시 범위를 넓히지 않는다.
...
// 두 번째로 같은 결정을 내렸다("한 글자도 남기지 마"). 그때는 `limit`(고지
// ⑤)을 소유자가 인용한 대상이 아니라며 남겼다.
//
// **D61(관리자 판정, 소유자 지시, 세 번째 같은 방향) — 이번에 `limit`이
// 지목됐다.** 세 문장과 그것을 담던 `limitNote()`·`.limit-note` 칸을 함께
// 지웠다. 남은 `DISCLOSURE` 객체가 `limit` 하나뿐이었으므로 객체 자체도
// 걷는다 — 쓰이지 않는 값을 코드로 남기지 않는다.
//
// 되돌려야 하는 조건(`open_questions`, D60에서 넘어옴): 자격자 관여를
// 시사하는 표시가 화면에 하나라도 생기면 이 문장들이 다시 필요해진다.
 ...
 *
 * **[2026-08-18, 관리자 지시(4차) 6번, D74] 「요약 저장」으로 다시 갈린다.**
 * 소유자가 이미지(PNG)를 다시 요구했고, 이번에는 페이지 전체가 아니라
 * **요약**(도넛·계좌별 월 납입액·총 절세액·계좌별 납입 잔여 한도·연 환산)
 * 만 담는다 — 옛 "공유용 이미지"(전체 페이지 캔버스, D10에서 걷어냄)의
 * 부활이 아니다. PDF도 같은 요약만 찍도록 인쇄 CSS를 좁혔다
 * (`styles.css` `@media print`, `ui/result-panel.js`의 `summarySheet`).
 * 이미지·PDF 둘 다 같은 요약을 담으므로 안내 문구도 하나로 합친다.
 */
export const IMAGE_EXPORT_LABEL = '이미지로 저장';
export const PDF_EXPORT_LABEL = 'PDF로 저장';
/** 이미지·PDF 버튼 위 소제목. 소유자 지시 문구("요약 저장")를 그대로 쓴다. */
export const SAVE_SHARE_HEADING = '요약 저장';
// [2026-08-18, D74] 옛 문구("인쇄 대화상자의 …")는 PDF 한 종류만 가정했다 —
// 이제 이미지·PDF 둘 다 페이지 전체가 아니라 **요약**만 담으므로, 그 범위를
// 두 버튼이 함께 읽는 자리에 한 번만 적는다.
//
// **[2026-08-18, 관리자 지시(6차) 2번, D75] "입력값은 포함되지 않습니다"가
// 뒤집혔다.** 소유자가 D74의 유보를 명시로 덮어 요약에 입력값(생년월일·
// 총급여액·월 납입액 등)을 넣으라고 지시했다(D75) — 문구도 그 사실을 반영해야
// 한다, 그러지 않으면 화면 문구와 실제 산출물이 어긋난다. **입력하지 않은
// 항목은 표시되지 않는다는 것(D75가 남긴 선 ①)도 문구가 함께 말한다.**
export const SUMMARY_EXPORT_NOTE =
  '이미지·PDF 모두 요약(도넛·계좌별 월 납입액·총 절세액·계좌별 납입 잔여 한도·연 환산)과 입력하신 값(생년월일·총급여액·월 납입액 등, 입력하지 않은 항목은 표시되지 않습니다)을 담습니다.';
...
// 이미지 래스터라이즈 자체가 실패한 드문 경우(예: 캔버스를 지원하지 않는
// 아주 오래된 브라우저)에만 보인다 — 다운로드 자체가 막히는 경우(샌드박스
// iframe)는 예외를 던지지 않으므로 이 문구가 그 실패를 감지하지 못한다
// (`ui/summary-image.js`의 `downloadDataUrl` 머리말 — 실제 검증은
// dev server에서 한다).
export const IMAGE_EXPORT_BLOCKED_NOTE = '이미지를 만들지 못했습니다. 잠시 후 다시 시도해 주세요.';

/**
 * `[7번, D74]` 결과 공유 링크. URL **프래그먼트**(`#...`)에 입력값을 싣는다 —
 * 쿼리 문자열이 아니다(D74: 쿼리는 GitHub Pages 서버 로그에 도달해 "입력값은
 * 서버로 나가지 않는다"는 원칙이 깨진다. 프래그먼트는 브라우저가 서버로
 * 보내지 않는다). **공유 시점에 이 안내가 보여야 한다는 것이 D74의 조건이다**
 * — 모르고 누르는 공유는 동의가 아니다.
 */
export const SHARE_LINK_LABEL = '내 결과 공유하기';
export const SHARE_LINK_COPIED_NOTE =
  '이 링크에는 입력하신 값이 들어 있습니다. 링크가 복사되었습니다 — 원하는 곳에 붙여넣어 공유하세요.';
// 클립보드 자동 복사가 막힌 환경(권한 거부·샌드박스 iframe)에서 보인다 —
// 호출부가 링크 주소 자체를 이 문구 뒤에 이어 붙인다.
export const SHARE_LINK_COPY_BLOCKED_NOTE =
  '이 링크에는 입력하신 값이 들어 있습니다. 자동 복사에 실패했습니다 — 아래 주소를 직접 복사해 주세요.';
// 공유 링크로 페이지를 열었는데 프래그먼트를 읽지 못했을 때(버전이 다르거나
// 손상된 링크) 보인다. 조용히 무시하지 않고 짧게 알린다(D74).
export const SHARE_LINK_INVALID_NOTE = '공유 링크를 읽지 못했습니다 — 이전 버전이거나 손상된 링크입니다. 값을 직접 입력해 주세요.';
// [2026-08-23, D84 판정 1] 옛(v:1) 공유 링크가 가리키던 「절세계좌
// 계산기2」(옛 근거판) 탭이 지워지면서, 그 탭에만 있던 몇몇 입력(청년
// 자기신고·당해연도 누적 납입액·ISA 전환·정산 기간 등)은 이제 실을 자리가
// 없다. 링크 자체를 무효로 돌리지 않고(D84 판정 1 — "링크가 죽는 것보다
// 낫다") 지금 탭이 받는 값만 채운 뒤, 못 실은 값이 있다는 사실만 짧게
// 알린다 — 조용히 버리지 않는다.
export const SHARE_LINK_PARTIAL_NOTE =
  '이 공유 링크는 이전 화면 기준입니다 — 지금 화면이 받는 값만 채웠고, 일부 값(예: 청년 자기신고·올해 이미 넣은 금액 등)은 싣지 못했습니다.';
...
// ---------------------------------------------------------------------------
// [2026-08-18, 관리자 지시(6차) 2번, D75] 요약(이미지·PDF) 도넛 오른쪽의
// 입력값 블록.
//
// **D74는 "요약에는 원시 입력이 없다"였고, 그 상태를 유지한다는 것이 확장에
// 별도 판정이 필요한 이유였다.** 소유자가 그 별도 판정을 D75로 내렸다 —
// 도넛 오른쪽에 사용자가 입력한 값(생년월일·총급여액·월 납입액·켰다면
// 예상 수익률·수익 성격·정산 기간)을 적으라고 지시했다. **받아들이는 근거는
// D75 본문 그대로다 — 요약은 사용자가 자기 기기에 저장하는 자기 파일이고,
// 값은 시트 위에 눈에 보이게 실린다. 우리가 전송하는 것이 없다는 사실은
// 변하지 않는다.**
//
// **유지되는 선 — 입력하지 않은 값은 줄 자체가 없다.** 이 라벨 객체는
// 여섯 항목의 "이름표"일 뿐이고, 실제로 어느 줄이 그려지는가는
// `ui/summary-data.js`의 `buildSummaryInputs`가 항목마다 값이 실제로
// 있는지를 보고 정한다 — 라벨이 여기 있다고 항상 그려지는 것이 아니다.
// ---------------------------------------------------------------------------
export const SUMMARY_INPUTS_HEADING = '입력값';
export const SUMMARY_INPUT_LABEL = {
  birth_date: BIRTH_DATE_LABEL,
  current_salary: '총급여액',
  monthly_capacity: '월 납입액',
  isa_return_rate: ISA_RETURN_RATE_LABEL,
  isa_income_character: '수익 성격',
  isa_settlement_years: '정산 기간',
};
