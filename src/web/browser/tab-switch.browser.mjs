 ...
 * 검사를 이 파일이 새로 진다.
 *
 * 남는 탭 구조: [절세계좌 계산기(id `calc2`)] [연금고갈 시뮬레이션(id
 * `pension-depletion`)] — 계산기2가 이제 유일한 계산 탭이라 라벨에서
 * "2"가 빠졌다(이미 D81에서 그렇게 됐다 — 이 회차가 새로 붙인 것이 아니라
 * 그 상태가 이제 "유일한 계산 탭의 이름"이라는 뜻을 추가로 진다).
 */
...
test('탭이 정확히 둘, 순서가 [절세계좌 계산기, 연금고갈 시뮬레이션]이다(D84 판정 1·2)', { skip: skipWithoutChrome }, async () => {
  const { page } = app;
...
test('.app-main이 정확히 둘 있다 — 탭마다 하나씩, 동시 마운트(2.1.2절 (3))', { skip: skipWithoutChrome }, async () => {
  const { page } = app;
  ...
  assert.equal(initial.calc2Hidden, false, '기본 탭(calc2)은 처음부터 보여야 한다');
  assert.equal(initial.depletionHidden, true);
});
...
test('탭을 전환하면 URL 프래그먼트가 탭 id로 바뀐다(2.1.2절 (4))', { skip: skipWithoutChrome }, async () => {
  ...
  const state = await page.evaluate(`(() => ({
    ...
    calc2Hidden: document.getElementById('tabpanel-calc2').classList.contains('tab-panel-hidden'),
    depletionHidden: document.getElementById('tabpanel-pension-depletion').classList.contains('tab-panel-hidden'),
    ariaSelected: document.getElementById('tab-pension-depletion').getAttribute('aria-selected'),
  }))()`);
  assert.equal(state.hash, '#pension-depletion');
  assert.equal(state.calc2Hidden, true);
  assert.equal(state.depletionHidden, false);
  assert.equal(state.ariaSelected, 'true');

  await page.clickElement(`document.getElementById('tab-calc2')`);
  await sleep(150);
  assert.equal(await page.evaluate(`location.hash`), '#calc2');
});
...
/** [2026-08-20, 관리자 지시(2차) 6번] 로고 클릭 — 첫 랜딩(calc2 활성 + 맨 위)으로. 입력값은 지우지 않는다. */
test('로고를 누르면 첫 탭(calc2)이 활성화되고 페이지 맨 위로 스크롤된다(입력값은 지우지 않는다)', { skip: skipWithoutChrome }, async () => {
  ...
  assert.equal(after.calc2Hidden, false);
...
test('D84 판정 1 — 예시 팝업이 기본 랜딩(프래그먼트 없는 첫 로드)에서도 뜬다', { skip: skipWithoutChrome }, async () => {
  const { page, origin } = app;
  await page.goto(`${origin}/src/web/index.html`);
