 ...
 * 패널·역산기 탭과 같은 절댓값 폰트(`SLICE_LABEL_NAME_FONT_PX`, 신규
 * 회차(×2)에서 15→16.5px로 +10%, 비율 줄은 -2=14.5px)를 쓴다 — 더는
 * 1.5배가 아니다. **항상 고리 밖(리더선)
...
test('D79 판정 4 — 도넛 조각 라벨이 이름+비율이고 항상 고리 밖(리더선)이며, 중앙 라벨은 예시 축소 배수를 받는다', { skip: skipWithoutChrome }, async () => {
  ...
  // [2026-08-23, 소유자 지시 1번으로 상한만 남기고 하한을 열었다, 신규
  // 회차(×2)로 기준값 갱신] D79는 "1.5배가 없어졌다"(더 커지지 않는다)는
  // 것이었지, "글자 크기가 절대 안 줄어든다"는 약속이 아니었다 — 그런데
  // "라벨이 카드 밖으로 잘리면 안 된다" 요구(`ui/charts.js`)가 고리 밖으로
  // 크게 밀려나야 하는 넓은 라벨(예: "연금저축" 네 글자 + 두 자리 %)에서는
  // 기본 크기 그대로 두면 카드 여백을 아무리 늘려도 못 맞추는 극단이
  // 실측으로 나왔다 — 그 라벨만 조각 안 라벨(`fitsInsideSlice`)과 같은
  // 방식으로 글자를 줄여 카드 안에 온전히 들어가게 한다
  // (`SLICE_LABEL_MIN_FONT_PX`=9.9px가 바닥). 신규 회차 지시로 기본
  // 크기가 15→16.5px(+10%)로 올랐으므로 이 시험의 상한도 함께 올린다 —
  // "16.5px를 넘지 않는다"(D79의 핵심 — 1.5배 금지, 이제는 그 기준값
  // 자체가 16.5)만 지키고, 9.9~16.5px 범위를 허용한다.
  for (const [i, size] of m.nameFontSizes.entries()) {
    const v = px(size);
    assert.ok(v >= 9.9 - 0.5 && v <= 16.5 + 0.5, `${i}번 조각 이름 라벨(${size})이 9.9~16.5px 범위 밖이다`);
  }
  for (const [i, size] of m.pctFontSizes.entries()) {
    const v = px(size);
    assert.ok(v >= 9.9 - 0.5 && v <= 14.5 + 0.5, `${i}번 조각 비율 라벨(${size})이 9.9~14.5px 범위 밖이다`);
  }
