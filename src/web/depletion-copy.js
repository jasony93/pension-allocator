...
/** [2026-08-25, 소유자 지시 1번] 궤적 시작점(2026) 라벨 — "현 적립금
 * {값}조원". 값 자체는 `depletion/constants.js`의
 * `INITIAL_FUND_TRILLION_KRW`(전망 재현 출발값)에서만 온다 — 실적
 * (`ACTUAL_FUND_BALANCE`)과 섞지 않는다(D84 출처 분리, 소유자 지시 2번
 * 원문 그대로 유지). 문구만 "적립금 …(전망 기준)"에서 "현 적립금 …"으로
 * 바뀌었다 — 위치도 그래프 궤적과 겹치지 않게 y축 위쪽으로 옮겼다
 * (`ui/depletion-panel.js`의 `buildDepletionChart`). */
...
//
// [2026-08-25, 관리자 지시 — 카피 검증(33.11절) 결과 반영] 본문 문구
// (질문·답·2064년 문장·계좌 안내)는 검증대로 전부 참이라 **그대로** 둔다.
// 바뀐 것은 출처 표기 하나뿐이다:
//  1. **출처를 두 줄로 나눈다** — 한 줄이면 2064년이 그 조항에서 나온
//     것처럼 읽힌다. 지급보장 문장의 출처(제3조의2)와 2064년 문장의
//     출처(보건복지부)는 서로 다른 문서다.
//  2. 제3조의2를 「신설」로 읽히게 하지 않는다 — 2014년 신설·2025년
//     **개정**(「지급되도록」→「지급을 보장」 강화)이다. 카피 원문에
//     "신설"이라는 말 자체가 없으므로 그대로 두되, 이 사실을 여기 주석으로
//     남겨 나중에 부연을 달게 되면 실수로 "신설"을 쓰지 않게 한다.
//  3. **2064 vs 이 시뮬레이터의 결과를 잇는 한 줄을 추가한다** — 정부
//     추계(2064년, 2024년 말 기금 기준)와 이 시뮬레이터의 소진 연도가
//     다르게 보이는 것은 등식이 달라서가 아니라 **출발 잔고가 다르기
//     때문**이라는 사실(검증 확인 — 출발 적립금을 1,200조로 되돌리면
//     이 모델도 2065년을 내 정부 추계와 +1년 오차로 좁혀진다)을 화면에
//     밝힌다 — 없으면 사용자가 카피의 2064년과 화면의 소진 연도 중
//     하나를 오류로 읽는다.
// ---------------------------------------------------------------------------
...
// 본문 — 검증대로 참이라 그대로 둔다(2064년 수치 손대지 않음).
export const DEPLETION_POPUP_BODY_LINE_1 = '기금 소진은 2064년으로 미뤄졌을 뿐입니다.';
export const DEPLETION_POPUP_BODY_STRONG = '국민연금은 바닥이지, 전부가 아닙니다.';
 ...
 * 소진 연도가 다르게 보이는 이유 — 등식이 아니라 출발 잔고 차이라는 것을
 * 밝힌다. 이 시뮬레이터의 출발값(1,458조)은 `depletion/constants.js`의
 * `INITIAL_FUND_TRILLION_KRW`와 같은 값이다(문구 안 숫자를 두 곳에서
 * 따로 관리하지 않으려면 그 상수를 바꿀 때 이 문장도 함께 봐야 한다 —
 * TODO(tax-domain 정정 시): 상수가 바뀌면 이 문장의 "1,458조"도 같이 검토). */
export const DEPLETION_POPUP_RECONCILIATION_NOTE =
  '정부 추계는 2064년(2024년 말 기금 기준), 이 시뮬레이터는 최신 적립금(1,458조)에서 출발해 그보다 늦게 나옵니다.';

/** [관리자 지시 — 카피 검증 1번] 출처 두 줄 — 지급보장 문장과 2064년
 * 문장은 서로 다른 문서에서 왔다. 하나로 뭉치면 2064가 조문에서 나온
 * 것처럼 읽힌다. */
export const DEPLETION_POPUP_SOURCE_GUARANTEE = '국민연금법 제3조의2 (2026.1.1 시행) — 지급보장 문장의 출처';
export const DEPLETION_POPUP_SOURCE_PROJECTION = '보건복지부, 2025.3 — 2064년 소진 전망의 출처';
