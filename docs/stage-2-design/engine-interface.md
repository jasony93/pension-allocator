...
open_questions:
  ...
  - "**[D77·`product-planner`·관리자 판정이 필요하다] requirements 9.1절에 없는 입력 둘을 계약이 받는다.** ① **`as_of_date`** — 필수다. 적립 기간이 「오늘」의 함수인데 순수 함수는 시계를 읽을 수 없다. 화면이 채우는 값이고 사용자에게 묻지 않는다. ② **`accounts.isa.cumulative_contribution_krw`** — 선택이다. ISA 그 해의 납입 한도 산식(`2,000만 × [1 + 경과연수] − 누적`)과 원금 초과 인출의 해지 의제 판정(GC-P9)이 이 값을 요구한다. **`null`이면 0으로 보고 가정을 내며, 그 방향은 한도를 크게 잡는 쪽**이라 「세 계좌만으로 달성할 수 없습니다」가 덜 나간다. **화면이 이 칸을 실제로 물을지는 `product-planner` 판정이다** — 9.1절의 「결과를 안 바꾸는 값은 안 묻는다」와 저울에 올려야 한다."
  - "**[D77·`tax-domain` 확인이 필요하다] 개시 시점을 「그 해의 생일」로 정한 것은 이 유닛의 판단이다.** 관리자가 개시를 **나이 또는 연도**로 받으라고 했고, 둘 다 하나의 날짜로 모아야 만 나이와 연령별 원천징수세율이 정해진다. 생일을 고른 이유는 (a) 이 저장소의 `ageOn`이 생일에 나이를 올리고 (b) **개시가 속한 과세기간 안에서 연령 구간을 넘는 일이 생기지 않아** 룰셋이 미확정으로 둔 「과세기간 중 생일을 지나는 해의 세율」을 피해 간다는 것이다. **세법이 정한 것이 아니므로 가정 코드로 낸다.** 다른 선택(연초·개시 신청일 등)을 택하면 경계에 선 사용자의 세율이 갈린다."
  - "**[D55 후속·`tax-domain`이 채울 자리를 값과 함께 적는다] 이월 판정에 딸린 연쇄 넷의 어휘를 열었다. 이 유닛은 정답지를 고치지 않았다.** `tax_liability_cap`에 `basis_rule_ids`·`basis_rule_ids_absent`를 새로 열었고(**포함 / 불포함** 검사다), D26 이래 비어 있던 `carryover_shares_future_year_credit_limit`·`carryover_requires_application`이 같은 연쇄에 붙는다. **채울 값**: **GC-32a·GC-32d**(둘 다 `applied: true`인데 이월 `false`) → 조건 둘 **`null`**, `basis_rule_ids_absent: [\"pension.credit.unused.contribution_carryover\"]`. **GC-31·GC-34**(이월 `true`) → 조건 둘 **`true`·`true`**(룰셋 `subject_to_conversion_year_credit_limits.value: true` · `automatic: false`이므로 `requires_application = !automatic`), `basis_rule_ids: [\"pension.credit.unused.contribution_carryover\"]`. **넷이 들어와야 「`applied`를 따라간 구현으로 되돌리면 정답지가 무는가」가 값으로 갈린다** — 지금은 어느 블록도 그 연쇄를 주장하지 않는다. 형식 검사가 어긋난 짝(이월 거짓인데 조건이 남음 등)을 대조 전에 거절하는 것은 `golden-block-format.test.mjs`에 결함 주입 여섯으로 고정했다."
  - "**[D55 후속·이 유닛의 검사에서 찾은 빠진 축] 블록 어휘가 두 자리에 손으로 적혀 있는데 그 둘을 맞대는 검사가 없었다.** `tax-domain`이 23차에 「어휘에 세 칸이 없다」고 신고했는데 **둘은 D26 이래 코드 어휘(`PLAN_TAX_CAP_KEYS`)에 있었다** — 없었던 것은 어휘가 아니라 그것을 알리는 문장(`golden-cases.md` 1-A절)이다. 실측하니 **코드에만 있고 문서가 안 알리는 어휘가 열 건이고 그중 아홉이 그대로 `VOCABULARY_DEBT`에 있다** — 여러 회차 「아직 안 채웠다」고 적어 온 빚의 대부분이 **채우지 않은 것이 아니라 있는 줄 몰랐던 것**이다. `src/engine/golden-vocabulary-doc.test.mjs`로 상시 검사를 세웠고 지금 어긋난 것을 `DOC_DRIFT_DEBT`에 이름과 사유로 적어 두었다. **1-A절을 고칠 자리는 `docs/stage-4-verification/`이라 `tax-domain`의 몫이다** — 넣을 이름: `scenario.pension_credit_ceiling` · `scenario.unapplied_proposed_rules` · `plan.headline_composite_total` · `assumption_based_isa_estimate`의 다섯(`principal_basis_code`·`return_accrual_code`·`is_lower_bound_for_aggregate_taxpayer`·`assumes_contract_held_to_settlement`·`axis_ceilings`) · `tax_liability_cap`의 넷. **뺄 이름**: `tax_liability_cap.known`(D39·D40에 폐기됐고 적으면 형식이 거절한다)."
  - "~~**[D54 집행·관리자에게 올린다] 정답지 두 자리가 값을 받아야 한다. 이 유닛은 정답지를 고치지 않았다.** `contribution_carryover_available`을 `applied`에서 떼어 **실제 잘림(표시 금액)**에 매달았고(계약 5.5절·0.22절), 골든 어휘 `tax_liability_cap.contribution_carryover_available`을 열었다. **채울 값은 22차 검증 리포트가 이미 산출해 두었다** — **GC-32d는 `false`**, **GC-34는 `true`**.~~ **[D55에 해소]** `tax-domain`이 두 자리에 값을 넣었고, **이 회차에 그 어휘를 `VOCABULARY_DEBT`에서 지웠다.** 지우려고 실행기를 돌렸더니 **같은 줄에 스물넷이 찍혀 있었다** — 갚힌 빚은 `t.diagnostic`으로 나가 실패가 아니므로 회차마다 인쇄되면서 아무도 멈추지 않았다. **밀린 스물셋을 함께 지웠다.**"
  - "**[D54의 파급] 계약을 `14.0.0`(major)으로 올렸다 — 금액은 한 원도 안 바뀌고 참·거짓 한 칸이 바뀐다**(0.22절). 규약이 「기존 필드의 의미 변경」을 major로 정하고, **금액을 안 바꾸는 변경이라 금액을 보는 어떤 검사에도 걸리지 않는다**는 것이 minor를 기각한 결정적 근거다. `src/web`의 목이 즉시 `schema_version_mismatch`로 멈춘다 — 이 유닛은 `src/web/`을 열지 않았고 마이그레이션은 `web-dev`의 몫이다. **목이 맞춰야 할 것**: 버전 문자열 `14.0.0`, `mock-engine.js`의 `contribution_carryover_available: capApplied`를 **실제 잘림 기준**으로 옮기는 것, 조건 둘과 `basis_rule_ids`가 그 칸을 따라 빠지는 것, 그리고 `applied: true`이면서 그 칸이 `false`인 응답(총급여 24,795,208 · 월 2,666,667). **화면 문구도 함께 옮긴다** — 이월·전환 안내는 `applied`가 아니라 그 칸에 매단다(계약 7절 금지 목록)."
  - "**[D53 집행·관리자에게 올린다] 정답지 네 건이 움직여야 한다. 이 유닛은 고치지 않았다.** GC-31·GC-32d·GC-34는 `plan_count`가 **3 → 2**이고(표시 공제액이 같고 IRP만 더 묶는 `annuity_savings_first`가 D53 3번으로 빠진다) 다른 값은 한 원도 안 움직인다. GC-32a는 IRP **3,000,000 → 2,999,994**, ISA **0 → 6**, `tax_credit_before_cap` **1,350,000/135,000/1,485,000 → 1,349,999/134,999/1,484,998**, `threshold_income_tax_krw` **1,350,000 → 1,349,999**, `credit_eligible_krw` **9,000,000 → 8,999,994**, `monthly_rounding_residual_krw` **0 → 12**, `fill_order`에 `isa: 3`이 는다. **`tax_credit`(1,349,999/134,999/1,484,998)과 `applied: true`·`binding_code: binds_provably`·`plan_count: 2`는 그대로다.** 값의 저자는 `tax-domain`이다."
  - "**[D53 2번의 파급·관리자 판정이 필요하다] `tax_liability_cap.applied`가 대는 상대를 「트림 전 금액」으로 옮겼다**(계약 5.5절·0.21절). 옮기지 않으면 **세액 한도에 소수부가 있으면서 무는 모든 사용자**에게 `applied: false`가 나간다 — 표시 기준 트림이 잘라 낸 뒤의 정확값을 한도 아래로 내리기 때문이다. 그러면 화면은 한도 때문에 IRP가 잘린 사람에게 「한도에 걸리지 않았습니다」를 적는다. **근거는 이 유닛이 지어낸 것이 아니라 정답지 GC-32d의 문장이다** — 「트림 후 값이 한도와 같아졌다고 해서 한도가 안 물었다고 적으면 원인과 결과를 뒤집는다」. **딸린 대가 하나를 함께 올린다** — `contribution_carryover_available`가 `applied`를 그대로 따르므로, **실제로 밀려난 납입액이 없는 사용자에게도 `true`가 나갈 수 있다.** 그 칸을 「반환된 배분에서 실제로 잘렸는가」로 갈라 둘지는 `tax-domain`·관리자 판정이다. **→ D54가 답했다(`14.0.0`·계약 5.5절·0.22절): 갈라 두고, 자는 표시 금액이다.** 시행령 §118의3①의 대상이 「받지 아니한 **금액**」이고 구조가 의제인출 + 의제재납입이라 그 금액이 계좌에 남아 있어야 하는데 **원 미만은 남을 수 없다.** **그 단계를 정하는 조문이 없다는 사실도 계약에 함께 적었다.**"
  - "**[D53 3번의 파급] 지배 판정이 `plans` 배열의 구성을 바꾼다.** 벡터가 다른데도 빠지는 안이 생겼으므로 **배열 길이를 고정으로 가정하거나 특정 `plan_id`가 반드시 있다고 가정한 화면은 조용히 깨진다.** 이 유닛은 `src/web/`을 열지 않았고 마이그레이션은 `web-dev`의 몫이다. **목이 맞춰야 할 것**: `schema_version` `13.0.0`, `plans`에서 `annuity_savings_first`가 빠질 수 있는 것, `within_isa_lock_in`인데 ISA에 돈이 들어간 응답, 그리고 `applied: true`이면서 `reduced_total_krw: 0`인 응답."
  - "**[게이트 4 직전·문서 개정] 계약의 값은 한 글자도 바뀌지 않았고 구조만 바뀌었다. `status`는 `draft` 그대로다.** 0절이 「버전 이력」에서 **「지금의 계약」**이 됐고(0.A 색인 · 0.B 지금 유효한 판정 · 0.C 폐기된 것 · 0.D 버전 규약), 버전 표와 판정 근거 0.1~0.19가 **11절 부록**으로 내려갔다. **소절 번호는 하나도 바꾸지 않았다** — 다른 문서와 `src/web`의 주석이 「계약 0.12절」처럼 이 번호로 가리킨다. **함께 고친 낡은 서술 넷**: 3.0절의 `schema_version` 예시가 `\"4.0.0\"`이었다(현재 11.0.0), 5.9절이 `4.0.0`에서 사라진 `profile.age_years`를 산식에 쓰고 있었다, 7.1절이 같은 필드를 오류 경로 예시로 들고 있었다, 10절이 「배분안이 여전히 셋」이라 적고 있었다(`5.0.0`부터 넷). **버전 규약을 0.4절 꼬리에서 0.D절로 옮겼다** — 규약을 가리키던 참조 하나가 `0.12절`이라는 틀린 번호를 적고 있었다."
  - "**`0.11`이라는 소절 번호가 둘이다**(11.B절). 하나는 `6.0.0`의 major 근거(D32)이고 하나는 D31이 남긴 되돌림 경로다. **`src/web`의 주석이 두 뜻 모두로 「0.11절」을 가리키고 있어**(`copy.js`가 앞엣것, `store.js`·`mock-engine.js`가 뒤엣것) 이 유닛이 임의로 다시 매기지 않았다. 다시 매기면 `src/web`의 주석 다섯 자리가 끊기고 이 유닛은 그 디렉터리를 열지 않는다. **번호를 바로잡을지는 관리자 판정이다.**"
...
**어느 쪽도 정본이 아니다.** 이 절과 11절의 서술이 정의 자리(3~10절)와 어긋나면 **정의 자리가 이긴다**(8.0절). 이 절은 색인이고 11절은 경위다.

**읽는 순서.** 처음이면 0.A → 3절(요청) → 5절(응답) → 8절(코드)이다. 11절로 갈 이유는 하나뿐이다 — **왜 그렇게 정했는지**를 알아야 할 때.

### 0.A 지금 응답에 무엇이 있는가 — 어디를 보면 되는가

**이 표는 색인이지 정의가 아니다.** 각 칸의 자료형·단위·필수 여부는 오른쪽 절이 정한다.

| 묶음 | 지금 오가는 것 | 정의 자리 |
|---|---|---|
| 요청 최상위 | `schema_version` · `tax_year` · `scenarios` · `profile` · `accounts` · `isa_transfer` · `options` | 3.0절 |
| 사용자 | `Profile` — 소득 넷, 자격 물음 둘, ISA·연금 관련 넷, 예산 둘, 수익률 가정 하나 | 3.1절 |
| 계좌 | `PensionAccountState`(둘) · `IsaAccountState` | 3.2절 |
| ISA 만기 전환 | `IsaTransfer` | 3.3절 |
| 수익률 가정 | `IsaReturnAssumption` | 3.6절 |
| 옵션 | `Options` — 배분안 목록 · 근거 포함 여부 · 정산액 표시 억제 | 3.4절 |
| 응답 최상위 | `ok` · `echo` · `scenarios` · `assumptions` (실패면 `errors`) | 4.1절 |
| 되돌림 | `Echo` — 공제율 구간, 엔진이 만든 만 나이, **세 개의 `*_affects` 고정 객체** | 4.2절 |
| 시나리오 | `ScenarioResult` — 아래 묶음들을 담는 껍데기 | 5절 |
| 자격 (두 축) | `AccountEligibility`(계좌를 **열 수 있는가**) · `PensionCreditTaxpayerEligibility`(공제를 **받을 수 있는가**) | 5.2절 · 5.18절 |
| 한도 | `LimitBreakdown` · `AccountLimit` · `IsaTransferExtraLimit` | 5.3절 · 5.4절 |
| 세액 한도 | `TaxLiabilityCap`(시나리오 단위, 산출 중간값 포함) · `PlanTaxLiabilityCap`(배분안 단위) | 5.10절 · 5.5절 |
| 배분안 | `Plan` · `Allocation` · `PriorityBasis` · `TieBreak` · `DeterministicBenefit` · `NonQuantifiedEffect` · `PlanWarning` | 5.5절 · 5.6절 |
| 미배분 | `UnallocatedBreakdown` | 5.13절 |
| 가정 위의 ISA | `AssumptionBasedIsaEstimate` | 5.14절 |
| 화면 눈금 | `PensionCreditCeiling`(세액공제 축의 끝) · `HeadlineCompositeTotal`(헤드라인 합계) | 5.15절 · 5.17절 |
| 세율 참고표 | `PensionWithdrawalTaxReference` — **혜택 금액 칸이 없다** | 5.16절 |
| 근거·고지 | `LegalBasisEntry` · `UncertaintyNote` · `UnappliedRule` · `Assumption` · `Notice` | 5.7절 · 5.8절 · 4.3절 · 7.2절 |
| 시점 | `PensionWithdrawalStart` · `FundUseHorizonBoundaries` | 5.11절 · 5.9절 |
...
**각 행의 「지금의 답」은 현재 버전에서 참인 진술이다.** 그 답이 언제·왜 그렇게 정해졌는지는 마지막 열의 판정 번호와 11절이 말한다. **코드가 언제 나가는지는 여기 적지 않는다** — 그 자리는 8.1~8.5절이다(8.0절).

| 물음 | 지금의 답 | 값이 나가는 자리 | 정의 자리 · 판정 |
|---|---|---|---|
| 세액 한도는 상한인가 등식인가 | **상한이다.** 실제 한도는 이 값보다 작거나 같고 **절대 크지 않다.** 등식이 되는 자리는 하나뿐이다 — 상한이 0이면 실제 한도도 0이다 | `cap_krw` · `is_upper_bound` · `is_exact` · `error_direction_code` | 5.10절 · **D39 · D40** |
| 미정 분기에서 화면이 「최대」를 쓸 수 있나 | **못 쓴다. 「적어도」도 못 쓴다.** 근로소득 외 합산 소득이 있다고 답했는데 금액을 모르면 오차의 **부호가 정해지지 않는다.** 숫자는 나가되 방향은 말하지 않는다 | `error_direction_code: "direction_indeterminate"` · 안내 `tax_liability_cap_direction_indeterminate` | 8.7절 · **D41 2번** |
| 원 미만은 어느 단계에서 몇 번 버리나 | **두 번이고, 성질이 다르다.** ① 조문이 정한 자리 한 번 — **과세표준**(「국고금 관리법」 §47② · 1원 미만 버림). ② 우리 규약으로 한 번 — 응답에 **정수 원으로 실릴 때.** 중간값과 **대소 비교에서는 버리지 않는다.** §47①(10원)은 우리 출력에 걸리지 않으므로 **응답 금액은 10의 배수가 아니다** | `tax_base_krw`(①) · 가정 `rounding_floor_to_won`(②) · 룰셋 `tax.rounding.won_fraction` | `engine-design.md` 13절 · **D46 1번** |
| `applied: false`가 「한도에 안 걸렸다」인가 | **아니다.** 「우리가 아는 상한으로는 안 잘렸다」일 뿐이고 실제 한도는 더 작을 수 있다. 걸림이 **증명되는** 상태는 하나뿐이다 | `binding_code` (`binds_provably` / `binding_not_determined`) | 8.7절 · **D40** |
| `applied === (잘린 금액 > 0)`인가 | **아니다.** `applied`는 **트림 전 금액과 한도의 정확값 대소**이고(`13.0.0`) 잘린 금액은 **반환된 배분의 표시 금액 뺄셈**이다. 화면이 「잘렸습니다」를 쓸 때는 `reduced_total_krw > 0`을 함께 본다. 한 방향만 무조건 참이다 — **금액이 줄었으면 반드시 잘린 것이다** | `applied` · `reduced_*_krw` | 5.5절 · **D46 1번 · D49** |
...
| 배분 금액을 바꾸는 입력은 무엇인가 | **`12.0.0`에서 둘이 바꾸게 됐다.** `fund_use_horizon`이 `within_isa_lock_in`이면 **연금 두 계좌가 미배분**이고(D10을 뒤집었다), 세액 한도는 **기본안 후보의 IRP 배분 상한**을 정한다. **수익률 가정은 여전히 아무것도 안 바꾼다.** 세 고정 객체가 그 선언이고 불변식이 응답 전체에서 대조한다 | `echo.fund_use_horizon_affects` · `echo.tax_liability_cap_affects` · `echo.isa_return_affects` | 4.2절 · **D52 · D28** |
| 3년 안에 쓸 돈이면 어디에 넣나 | **연금 두 계좌에는 넣지 않는다.** 55세 전 인출이라 기타소득세가 붙고 그 세율이 공제율보다 낮지 않아 **받은 것을 도로 내거나 더 낸다.** **ISA는 남은 의무가입기간이 있을 때만 함께 비운다**(`13.0.0`) — 기간이 이미 지난 사용자에게는 추징 요건이 **성립할 수 없으므로** 배분을 유지한다. 그 사실이 미배분의 **이유 코드**로 나간다 | `unallocated_breakdown.reason_code` · `allocations[].limited_by: "fund_use_horizon"` · `fund_use_horizon_boundaries.isa_lock_in_years_remaining` | 3.1절 · 5.13절 · **D52 2번 · D53 1번** |
| 결정세액이 낮은 사람에게 IRP를 권하나 | **권하지 않는다.** **표시되는 세액공제액**을 한 원도 늘리지 않는 IRP 배분을 기본안 후보가 내지 않는다(`13.0.0` — `12.0.0`은 정확값으로 재었다) — 얻는 것 없이 중도인출 제한만 지기 때문이다. **연금저축은 다르다**(그 제한을 받지 않고 이월 신청이 예정된다). **경계는 총급여가 아니라 남은 세액 한도가 정한다** | `allocations[].limited_by: "no_additional_tax_credit"` | 5.5절 · 5.12절 · **D52 1번 · D53 2번** |
| 고를 이유가 없는 안이 선택지에 남나 | **남지 않는다**(`13.0.0`). **표시되는 세액공제액이 같고 연금저축·ISA도 더 받지 못하면서 IRP만 더 묶는 안**은 `plans`에서 뺀다. 사용자가 그 안을 골라 얻는 것이 중도인출 제한뿐이기 때문이다. 「목적」은 이름이 아니라 **다른 값이 실제로 달라지는 것**으로 잰다 | `plans` 배열의 길이와 구성 | 5.5절 · 6.2절 · **D53 3번** |
| `applied`는 무엇에 대고 재나 | **트림 전 금액에 댄다**(`13.0.0`). 한도가 물었기 때문에 IRP를 잘라 내는데, 잘라 낸 **뒤**의 금액과 대면 자름이 사라져 보인다. **`reduced_*`와 `threshold_income_tax_krw`는 여전히 반환된 배분의 표시 금액**이므로 `applied: true`이면서 `reduced_total_krw: 0`인 자리가 실재한다 | `tax_liability_cap.applied` · `reduced_*_krw` | 5.5절 · **D53 2번** |
| 이월할 납입액이 있는가는 무엇으로 재나 | **표시 금액으로 잰다**(`14.0.0`). `applied`를 따르지 않는다 — 두 칸이 **다른 물음**에 답한다. `applied`는 「한도가 물었는가」이고 이 칸은 「**밀려난 납입액이 있는가**」다. 시행령 §118의3①의 대상이 「받지 아니한 **금액**」이고 구조가 **의제인출 + 의제재납입**이라 그 금액이 계좌에 남아 있어야 하는데 **원 미만은 남을 수 없다.** **그 단계를 정하는 조문은 없다** — 우리가 정한 자다. **`applied: true`이면서 이 칸이 `false`인 자리가 실재하고 그것이 설계다** | `contribution_carryover_available` · `carryover_*` | 5.5절 · **D54** |
...
| `legal_basis`의 「사전순」은 무엇인가 | **코드 단위 비교**다(`Array.prototype.sort()`의 기본 순서). `localeCompare`가 아니다 | `legal_basis` 배열 순서 | 6.1절 · `10.0.0` |
| 월 표시 금액의 합은 월 여력과 같은가 | `monthly_unassigned_krw`가 0이면 **정확히 같다.** `monthly_krw`는 순수한 내림이 **아니다** — 잔차를 떠안는다. 화면에서 반올림해 메우지 않는다 | `monthly_krw` · `monthly_rounding_adjustment_krw` · `monthly_unassigned_krw` | 5.5절 · 11절 0.12 |
| 역산기에서 수익률을 안 주면 0으로 계산하나 | **아니다. 그 블록이 통째로 없다.** `null`은 「효과가 0」이 아니라 「묻지 않았다」이고, 0은 **사용자가 준 값일 때만** 계산된다 | `contribution_scenario: null` · `contribution_scenario_absent_reason_code` | 12.4절 · 8.11절 · **D77 판정 1** |
| 역산기의 법정 판정이 수익률을 타나 | **타지 않는다.** 필요 최소 평가액과 성립 여부는 **무성장 기준으로 고정**된다 — 가정이 「연금수령인가 아닌가」라는 조문 판정의 참·거짓을 바꾸는 계산(층 4)을 하지 않는다 | `echo.return_rate_affects`의 앞 두 칸이 언제나 `false` | 12.5절 · 12.6절 · **D77 판정 1** |
| 1,500만 문턱과 연금수령한도는 같은 규칙인가 | **정반대다.** 문턱은 **요건**이라 넘으면 **전액**이 밖으로 나가고, 한도는 금액의 **상한**이라 넘으면 **초과분만** 성격이 바뀐다. **같은 규칙으로 처리하면 반드시 한쪽이 틀린다** | `threshold_consumption.elective_base_krw`(전액) · `minimum_start_balance.binding_code` | 12.6절 · 정답지 GC-P3 · GC-P6 |
| 역산기가 어느 수령 전략을 권하나 | **어느 것도 권하지 않는다.** 연금 외 소득을 모르면 유불리를 **계산조차 하지 않고**, §64조의4의 두 읽기가 갈리면 값은 내되 결론을 내지 않는다 | `comparison_code` · `lower_option_code`(`null`) | 12.8절 · 8.11절 · **AC-R10** |

### 0.C 지금은 없는 것 — 폐기된 입력·필드·코드

**이 표가 있는 이유.** 사라진 이름을 목이나 화면이 아직 들고 있으면 그것은 **조용히** 낡는다. 이 저장소에서 실제로 그런 자리가 나왔고(`qa` 리포트 11.3절), 「없다」를 한 곳에 모아 두는 것이 그것을 찾는 가장 싼 방법이다.

| 사라진 것 | 사라진 버전 | 지금 그 자리에 있는 것 |
|---|---|---|
| `profile.risk_profile` | `2.0.0` (D10) | `profile.fund_use_horizon` |
| `profile.age_years` | `4.0.0` (D21) | `profile.birth_date` + `echo.derived_age` |
| `profile.prior_year_tax`와 `PriorYearTax` 타입 | `9.0.0` (D39 · D40) | **대신할 입력이 없다.** 이미 받고 있는 총급여액이 한도를 정한다(5.10절) |
| 배분안 id `pension_contribution_limit_fill` | `6.0.0` (D32) | `pension_contribution_before_isa` |
| `limited_by`의 `credit_limit` | `6.0.0` (D32) | `pension_combined_credit_remaining_after_plan_krw` · `pension_contribution_without_credit` |
| `TaxLiabilityCap`의 `known` · `determined_tax_krw` · `prior_pension_credit_krw` · `source_code` · `declared_nonzero` | `9.0.0` | `branch_code` · `is_upper_bound` · `is_exact`와 산출 중간값 여섯 |
| `PlanTaxLiabilityCap.known` | `9.0.0` | `binding_code` |
| 안내 코드 `tax_liability_cap_unknown` | `9.0.0` | `tax_liability_cap_estimated_from_total_salary` · `tax_liability_cap_direction_indeterminate` |
| 가정 코드 `prior_pension_credit_zero_assumed` | `9.0.0` | **없다.** 되더할 입력 자체가 사라졌다 |
| `tax_liability_cap_relation_code`의 `cap_unknown` | `9.0.0` | **없다.** 「모름」이라는 상태가 사라졌다 |
| `delta_vs_baseline_krw`의 「0 이하」 보장 | `3.0.0` | **없다.** 부호를 가정하지 않는다(11절 0.1) |
...
| `fund_use_horizon`이 배분 금액·세액공제액을 바꾸지 않는다는 보장 | `12.0.0` (D52 2번) | **세 값에서만** 참이다. `within_isa_lock_in`은 연금 두 계좌가 미배분이다(3.1절·5.13절) |
| `within_isa_lock_in`이면 **모든** 배분안의 세 계좌 금액이 0이라는 보장 | `13.0.0` (D53 1번) | **ISA는 남은 의무가입기간이 있을 때만** 0이다. 기간이 지났으면 배분을 유지한다(3.1절) |
| 「목적이 있는 배분안은 이름만으로 남는다」는 읽기 | `13.0.0` (D53 3번) | 표시 공제액이 같고 IRP만 더 묶는 안은 **이름과 무관하게** 빠진다(5.5절·6.2절) |
| `tax_liability_cap.applied`가 **반환된 배분의** 자르기 전 금액과 한도의 대소라는 보장 | `13.0.0` (D53 2번) | 대는 상대가 **트림 전** 금액이다. `applied: true`이면서 `reduced_total_krw: 0`인 자리가 생겼다(5.5절) |
| `contribution_carryover_available === applied` 보장 | `14.0.0` (D54) | 한 방향만 — **이월할 납입액이 있으면 반드시 한도가 물었다.** 반대는 성립하지 않는다(5.5절) |
...
**이 문서가 이 규약을 확정한다.** 어느 범주에 드는지의 실제 판정 사례는 11절에 버전마다 적혀 있다.
...
|---|---|---|---|---|
| `schema_version` | string | — | 필수 | **현재 `"11.0.0"`.** 0절 머리의 「현재 계약 버전」이 이 값의 정의 자리이고, `src/engine/constants.mjs`의 `SCHEMA_VERSION`과 같은지를 `scripts/org/validate-code-definitions.mjs`가 본다. major가 다르면 `schema_version_mismatch` 오류(0.D절) |
...
| `fund_use_horizon` | `"within_isa_lock_in"` \| `"before_pension_age"` \| `"at_or_after_pension_age"` \| `"unknown"` | — | **필수** | 이 자금을 언제 쓸 계획인가. **`12.0.0`부터 값 하나가 금액을 바꾼다** — `"within_isa_lock_in"`이면 **연금 두 계좌가 미배분**이고(D52 2번), **ISA는 남은 의무가입기간이 있을 때만 함께 비운다**(`13.0.0` · D53 1번). 나머지 세 값에서는 종전대로 배분안의 순서와 경고만 바꾼다(3.1절 아래 설명). 값을 모르면 `"unknown"`을 보낸다 — **엔진이 기본값을 만들지 않는다.** 목록 밖 값이면 `invalid_enum` 오류 |
...
| 값 | 의미 | 걸리는 중도 불이익 |
|---|---|---|
| `within_isa_lock_in` | ISA 의무가입기간 안에 쓸 가능성이 있다 | 연금 두 계좌는 언제나 — **그래서 배분하지 않는다**(`12.0.0`). **ISA는 `fund_use_horizon_boundaries.isa_lock_in_years_remaining > 0`일 때만**이고, 0이면 불이익이 성립하지 않아 **배분을 유지한다**(`13.0.0`) |
...
`within_isa_lock_in`에서는 **계좌마다 판정한다**(`13.0.0` · D53 1번). 근거는 조문이고, **조문이 두 계좌를 다르게 정하기 때문에 판정도 갈린다.**

- **연금저축·IRP — 언제나 배분하지 않는다.** 55세 전 인출이 연금외수령이라 기타소득세가 붙고(`pension.early_withdrawal.other_income_rate`), 그 세율이 세액공제율보다 낮지 않아 **받은 것을 도로 내거나 더 낸다.** 이 요건은 **ISA 의무가입기간과 아무 관계가 없으므로** 아래 예외에 영향을 받지 않는다.
- **ISA — `fund_use_horizon_boundaries.isa_lock_in_years_remaining > 0`일 때만 배분하지 않는다.** 근거 규칙(`isa.early_termination.clawback`)이 추징을 거는 요건이 **"의무가입기간이 되는 날 전" 해지**이고(`isa.account.requirements.min_contract_years`), **남은 기간이 0인 사용자에게는 그 요건이 성립할 수 없다.** 성립하지 않는 불이익은 배분을 비울 근거가 되지 못한다 — 그 사용자에게 「넣지 마세요」를 적으면 화면이 이유를 대지 못한다.

**같은 판정을 8.5절이 이미 하고 있었다.** `all_accounts_have_early_exit_penalty`를 끄는 조건에 「예: ISA 의무가입기간이 이미 지난 사용자」가 적혀 있다. **경고를 끄는 자리에서 보이는 사용자가 배분을 비우는 자리에서 안 보이면 둘 중 하나는 반드시 거짓이다** — `12.0.0`에서 그 상태로 한 회차를 났고 D53이 그것을 닫았다.

그때 달라지는 것은 이렇다.

- **비워지는 계좌**의 `annual_krw`·`monthly_krw`가 `0`이고 `fill_order`가 `null`이며 `limited_by`가 `"fund_use_horizon"`이다(자격이 없는 계좌는 `"not_eligible"`이 이긴다 — 그 사실은 이 판정과 무관하게 참이다).
- **살아남은 ISA는 다른 시점과 똑같이 채워진다.** `limited_by`는 실제로 막은 것(`"budget"` 또는 `"contribution_limit"`)이고 **`"fund_use_horizon"`이 아니다** — 시점이 그 계좌를 막지 않았기 때문이다.
- `unallocated_breakdown.reason_code`는 **열려 있으면서 비워지지 않은 계좌가 하나도 없을 때만** `"no_account_beneficial_within_fund_use_horizon"`이다. ISA가 살아남았으면 미배분의 이유는 한도이므로 `"contribution_room_exhausted"`다(5.13절·8.10절).
- 네 안의 배분 벡터가 같아지면 **하나로 합쳐진다**(6.2절). 연금이 전부 0이고 ISA가 한 갈래뿐이라 이 시점에서는 언제나 그렇다. `plans_collapsed_single`이 나간다.
- **연금계좌에는 경고가 붙지 않는다.** 넣지 않은 돈에는 경고가 붙지 않는다는 규칙(8.4절)이 그대로 걸린 결과다. **살아남은 ISA에도 붙지 않는다** — 그 경고의 조건이 잔여 기간 > 0이기 때문이다(8.4절). 세 계좌가 다 비는 경우에는 `all_accounts_have_early_exit_penalty`가 그 이유를 진다(8.5절).
- 안내 `budget_exceeds_all_limits`가 **나가지 않는다.** 연금 한도가 멀쩡히 남아 있으므로 그 말이 거짓이 된다.
- 가정 `fund_use_horizon_excluded_from_amounts`가 **나가지 않는다.** 그 시점에는 거짓이기 때문이다 — **ISA 하나가 살아남아도 거짓이다.**

**한도는 이 값에서도 그대로다.** `limits`·`pension_credit_tax_liability_cap`·`pension_credit_ceiling`은 네 값 어디에서나 같다 — 이 판정은 **배분 판정**이지 한도 판정이 아니다.
...
| `applied` | boolean | — | **세액 한도가 이 사용자에게 물었는가.** 잘린 상태와 잘리지 않은 상태의 문구가 다르다. **`13.0.0`부터 대는 상대가 「트림 전 금액」이다**(D53 2번) — 한도가 물기 때문에 IRP를 잘라 내는데(5.12절) 잘라 낸 **뒤**의 금액과 대면 자름이 사라져 보이고 **원인과 결과가 뒤집힌다.** `12.0.0`까지는 두 값이 우연히 같아 이 구분이 필요 없었다. **트림이 없었으면 두 값은 여전히 같다.** **`false`를 「걸리지 않았다」로 읽지 않는다** — 아래 칸이 그 구분을 낸다. **`11.0.0`부터 이 판정은 원 미만을 버리기 전의 정확값끼리 한다**(룰셋 `tax.rounding.won_fraction`의 `comparison` 단계). 그래서 **`applied === (reduced_total_krw > 0)`이 성립하지 않는다** — 잘린 양이 1원에 못 미치면 `true`인데 아래 세 칸이 전부 0이다. **화면이 「잘렸습니다」·「이 막대가 짧은 것은…」을 쓸 때는 `reduced_total_krw > 0`을 함께 보아야 한다.** 반대 방향은 무조건 참이다 — 금액이 줄었으면 반드시 잘린 것이다 |
...
| `reduced_income_tax_krw` | integer | 원/연 | 잘린 소득세분. **표시 금액끼리의 뺄셈이다**(자르기 전 − 자른 뒤). 정확값의 차이를 버린 값이 아니므로 위 `applied`와 어긋날 수 있고, 그 자리는 화면이 나란히 놓는 세 수가 서로 맞는 쪽을 골랐다 |
...
| `contribution_carryover_available` | boolean | — | 잘린 공제에 대응하는 **납입액**을 이후 과세기간으로 전환 신청할 수 있는가(시행령 §118의3). **`14.0.0`부터 `applied`를 따르지 않는다**(D54) — **`reduced_income_tax_krw > 0`이다.** 두 칸이 **다른 물음**에 답한다: `applied`는 「**한도가 물었는가**」를 트림 전 금액에 대고 **정확값**으로 재고, 이 칸은 「**밀려난 납입액이 있는가**」를 **반환된 배분의 표시 금액**으로 잰다. **한 방향만 참이다** — 이 칸이 `true`면 `applied`도 `true`다. 반대는 아니다(아래 「자가 표시 금액인 이유」). **"넣은 돈이 사라진다"는 틀린 문구다** — 정확한 서술은 "올해의 세액공제는 0이고 그 납입액은 신청을 통해 이후 과세기간으로 넘길 수 있다"이다. **이 이름 하나가 조건 둘을 감추고 있었다**(D26) — 아래 두 칸을 함께 읽어야 한다 |
...
| `basis_rule_ids` | string[] | — | **`contribution_carryover_available`이 `true`일 때만** 전환 특례 규칙(`pension.credit.unused.contribution_carryover`)이 함께 실린다. `applied`가 아니라 그 칸을 따른다 — 읽지 않은 규칙을 근거로 싣지 않는다 |

**자가 표시 금액인 이유 — `applied`와 갈리는 것이 결함이 아니라 설계다** (`14.0.0` · D54)

**두 칸은 같은 사실의 두 이름이 아니다.**

| 칸 | 답하는 물음 | 재는 자 |
|---|---|---|
| `applied` | 이 사용자에게 **한도가 물었는가** | **트림 전** 배분의 자르기 전 소득세분 대 한도, **정확값** 비교 |
| `contribution_carryover_available` | **밀려난 납입액이 있는가** | 반환된 배분의 `reduced_income_tax_krw`, **표시 금액** |

**근거는 조문이다.** 시행령 §118의3①이 전환 신청의 대상을 「연금계좌세액공제를 받지 아니한 **금액**」으로 정하고, 그 구조가 **의제인출 + 의제재납입**이라 **그 금액이 계좌에 남아 있어야** 한다(룰셋 `pension.credit.unused.contribution_carryover`의 `mechanism.requires_amount_still_in_account`). **원 미만의 연금보험료를 「가장 먼저 인출하여 다시 납입한 것으로 본다」는 처분은 실행될 수 없다** — 계좌 잔액의 단위가 원이다. 실행될 수 없는 처분의 대상을 「있다」고 적으면 사용자를 없는 절차로 보낸다.

**그 단계를 정하는 조문은 없다.** 「국고금 관리법」 §47②가 지목하는 것은 **과세표준** 하나이고, 룰셋 `tax.rounding.won_fraction`의 표시 단계는 `determined_by_law: false`다. **이 조직이 정한 자이며, 계약이 그 사실을 여기에 명시한다.**

**정확값으로 재면 `13.0.0`이 닫은 구멍이 다시 열린다.** 어느 표시 금액에도 나타나지 않는 끝수를 「얻은 것」으로 세는 형태이고, `13.0.0`이 트림 기준을 표시 금액으로 옮겨 막은 바로 그 형태다.

| 좌표 | 정확값 초과분 | 표시 초과분 | `applied` | `contribution_carryover_available` |
|---|---|---|---|---|
| 총급여 24,795,208 · 월 2,666,667 | 0.045원 | **0** | `true` | **`false`** ← 두 자가 갈린다 |
| 총급여 30,686,275 (`CAP_COORDINATES.BINDS`) | 1원 | **1원** | `true` | `true` |

**화면이 어겨서는 안 되는 것.** 「다음 해로 넘길 수 있습니다」·「이월 신청」류의 문장은 **`applied`가 아니라 이 칸**에 매단다. `applied`에 매달면 **넘길 금액이 한 원도 없는 사용자**에게 신청 절차를 안내하게 된다. D49가 「이 막대가 짧은 것은…」을 `reduced_total_krw > 0`에 매단 것과 같은 판단이다.
...
**`13.0.0`에서 그 두 번째 이유의 조건이 좁아졌다** (D53 1번). 「어느 계좌도 이롭지 않다」는 **정말로 하나도 없을 때만** 참이다. ISA 의무가입기간이 이미 지난 사용자에게는 그 계좌가 이롭고 실제로 돈을 받으므로, **그 사용자의 남은 미배분은 시점이 아니라 ISA 납입 잔여 한도에서 멈춘 것**이고 `reason_code`가 `"contribution_room_exhausted"`다. 이 구분을 안 하면 화면은 **방금 돈을 넣은 계좌를 가리키며 「이로운 계좌가 없습니다」**를 적게 된다.
...
| `pension_years_remaining` | integer | 년 | 위에서 **엔진이 생년월일로 환산한 만 나이**(`echo.derived_age.age_years`와 같은 값)를 뺀 잔여. 0 미만이면 0. **`profile.age_years`는 `4.0.0`에서 사라졌다**(0.C절) — 화면이 나이를 계산해 보내지 않는다 |
...
| `tax_base_krw` | integer | 원/연 | 2단계 결과. 종합소득 과세표준. 0 미만이 되지 않는다. **이 칸만 조문이 끝수를 없앤 값이다** — 「국고금 관리법」 §47②(1원 미만 버림). 나머지 정수 금액은 조문이 지목하지 않은 자리이고 **표시 직전에 우리 규약으로 한 번 버린 값**이다(0.19절) |
...
**항등식.** `cap_krw = max(0, computed_tax_krw − wage_income_credit_krw)`이고 `tax_base_krw = max(0, (종합소득금액 또는 wage_income_amount_krw) − basic_deduction_krw)`다. **`11.0.0`부터 이 두 등식은 표시 금액끼리는 1원 어긋날 수 있다** — 실제 계산은 원 미만을 버리지 않은 정확값으로 하고 각 칸은 마지막에 한 번씩 버려 실리기 때문이다. 뒤 등식은 그대로 성립하지만(기본공제가 정수다) 앞 등식은 두 항의 끝수가 잘려 **좌변이 우변보다 1원 작을 수 있다.** **화면이 이 뺄셈을 하지 않는다** — 항을 전부 내는 이유가 그것이고, 한도가 이제 받은 값이 아니라 **계산한 값**이라 그 계산이 되짚어질 수 있어야 한다.

**네 좌표가 조문에서 검산돼 있다**(D40 · D46 1번). 총급여 **5,000,000원 이하 → 정확히 0**(과세표준이 정확히 0이므로 등식이다) · **30,686,275원 → 899,999원**(D40의 검산은 900,000이었다. §47②를 적용하면 과세표준이 19,333,333.75 → 19,333,333이 되어 1원 내려간다) · **34,143,911원 → 1,349,999원**(여기서 `applied`가 참이다) · **34,143,912원 → 정확히 1,350,000원.** 세 번째 값은 이 룰셋에서 **합산 인정한도를 최고 공제율로 전부 채운 세액공제 소득세분과 같다** — 즉 그 총급여 위로는 (ISA 만기 전환 추가한도가 없는 한) **이 한도가 아무것도 자르지 않는다.** 자르지 않는 것은 결함이 아니라 이 설계의 귀결이다.
...
- **다른 두 안은 트림하지 않는다.** 사용자가 다른 목적으로 고를 수 있고, 이번 변경은 **기본안이 그것을 앞세우지 않는다**는 것이다. **다만 `13.0.0`부터 그 안이 응답에 실린다는 보장은 없다**(6.2절 · D53 3번) — 트림하지 않는 것과 선택지로 내는 것은 다른 일이다.
- **기준은 표시되는 세액공제액이다**(`13.0.0` · D53 2번). `12.0.0`은 정확값으로 재었고, 세액 한도에 소수부가 있으면 그 기준이 **0.1원짜리 증가**를 「낳았다」로 세어 **15% 구간에서 1~6원**의 IRP를 남겼다. 그 몫은 `tax_credit`에도 `tax_credit_before_cap`에도 나타나지 않으므로 **얻은 것이 아니고**, 그것 때문에 중도인출이 제한되는 계좌에 돈을 넣게 하는 것은 이 절이 방금 금지한 바로 그 형태다. 비교는 **소득세분과 지방소득세분의 표시 금액 합**으로 한다.
- **`13.0.0`이 여기에 함께 적었던 「12% 구간 1~8원」은 거짓이 아니라 공허하다**(`14.0.1` · D55). **산술은 그대로 맞다** — 그 구간에서 한도가 소수부를 갖고 물면 잔여는 1~8원이다. **그런데 그 전제를 만족하는 좌표가 이 서비스에 없다.** 12% 구간(총급여 5,500만원 **초과**)에서 세액 한도의 **최솟값이 4,230,000원**(총급여 55,000,001원)이고 한도는 총급여에 대해 단조증가하는데, 같은 구간에서 나올 수 있는 **최대 공제액은 1,440,000원**이다(합산 공제한도 900만원 + ISA 전환 추가한도 300만원의 12%). **한도가 최대 공제액의 세 배라 물 수 없고, 물지 않으면 트림도 없다.** 그러므로 그 구간에서는 `tax_liability_cap.applied`가 언제나 거짓이고 트림 폭도 언제나 0이다.
  - **문장을 지우지 않고 공허하다고 적는 이유.** 「사는 사람이 없다」는 사실 자체가 값이다 — 지우면 다음 회차가 다시 그 구간의 트림 좌표를 찾으러 나선다. 실제로 22차가 그 좌표를 세우려다 이 성질을 알게 됐다.
  - **그 하한을 검사가 물고 있다.** 정답지 **GC-32e**가 12% 구간 최저 한도 좌표에서 4,230,000원을 값으로 못 박았으므로, 그 수가 내려가면 위 결론이 무너지기 전에 블록이 먼저 실패한다.
  - **경계를 총급여로 자르는 규칙이 새로 생긴 것이 아니다.** 트림 여부는 여전히 남은 세액 한도가 정하고(아래 줄), 계약도 엔진도 총급여 임계값을 갖지 않는다. 위는 그 규칙이 12% 구간에서 **무엇을 내는지**에 대한 서술이다.
- **그 트림이 `tax_liability_cap.applied`를 지우지 않는다**(5.5절). 한도가 물었기 때문에 잘라 낸 것이므로, 잘라 낸 뒤 값과 한도를 대어 「안 물었다」고 적으면 원인과 결과가 뒤집힌다.
...
**`12.0.0`부터 그 상태가 반드시 나오는 입력이 하나 있다** — `fund_use_horizon`이 `within_isa_lock_in`이면 연금 두 계좌가 어느 안에서도 0이고 ISA는 한 갈래뿐이라 **네 안의 벡터가 언제나 같아진다**(3.1절). 남는 하나는 `max_tax_credit`이고, 그 안의 이름이 이 상태를 설명하지 않는다는 것은 `unallocated_breakdown.reason_code`와 `all_accounts_have_early_exit_penalty`가 함께 말한다.

#### 6.2.1 목적 없이 IRP만 더 묶는 안은 내지 않는다 (`13.0.0` · D53 3번)

**벡터가 달라도 빠지는 안이 생겼다.** 어떤 안 P가 이미 남기기로 한 안 Q에 대해 아래 셋을 **동시에** 만족하면 `plans`에 싣지 않는다.

1. `deterministic_benefit.pension_credit_total_krw`가 **같다**(표시 금액끼리 비교한다 — 정확값으로 재면 어느 화면에도 안 나타나는 0.1원이 「다르다」가 되어 5.12절이 닫은 구멍이 여기서 다시 열린다),
2. `retirement_pension` 배분이 **더 많고**,
3. `annuity_savings`와 `isa` 배분이 **더 많지 않다.**

그 안이 사용자에게 주는 것이 **중도인출 제한뿐**이기 때문이다(`pension.withdrawal.midterm_restriction`). D52 후속은 「목적이 있는 안은 남긴다」고 정했고 **D53이 그 읽기를 깼다** — 절세액이 0인데도 IRP를 더 묶는 안이 이름만으로 남고 있었고, **절세액이 같으면 그 안의 목적이 절세일 수 없다.** 「목적」은 이름이 아니라 **다른 값이 실제로 달라지는 것**으로 잰다.

**연금저축이나 ISA가 한 원이라도 더 들어가면 빠지지 않는다.** 그때는 IRP 몫이 다른 무엇과 맞바꿔진 것이고, 그 맞바꿈의 값어치는 조문이 정하지 않으므로(과세이연·인출 자유) 엔진이 대신 판정하지 않는다.

**기본안은 이 판정으로 빠지지 않는다** — 판정의 상대는 언제나 **이미 남기기로 한 안**이고 기본안이 그 목록의 첫 항목이다.
...
|---|---|---|
| `code` | string | 8.1절 목록 |
| `field` | string \| null | 점 경로. 예: `profile.birth_date`, `accounts.isa.ytd_contribution_krw`. **지금 요청에 있는 경로만 예로 든다** — 사라진 필드를 예시로 남기면 그것이 아직 있는 것으로 읽힌다(0.C절) |
...
**0절도 11절도 정본이 아니다.** **0절은 색인**(지금 무엇이 있고 무엇이 유효한가)이고 **11절은 경위**(왜 그렇게 정했나)이며, 어느 쪽이든 서술이 정의 자리와 어긋나면 **정의 자리가 이긴다.** 11절의 서술은 적힌 시점의 사정을 담는다.
...
| `annuity_start_below_minimum_age` | **역산기 전용**(`14.1.0`, D77). `profile.annuity_start`로 정해지는 개시 시점의 만 나이가 `pension.withdrawal.eligibility`의 최소 개시 연령 미만. `params.minimum_age_years`(룰셋에서 읽은 값) · `params.start_age_years` · `params.basis_rule_ids`. **이 오류가 있으면 응답에 법정 사실 블록이 없다**(AC-R5) — 성립하지 않는 개시 시점 위에서 낸 「최소 평가액」은 사실이 아니다 |
...
| `budget_exceeds_all_limits` | info | 예산이 세 계좌 잔여 한도 합계를 넘음. **`fund_use_horizon`이 `within_isa_lock_in`이면 나가지 않는다**(`12.0.0`) — 그때는 연금 한도가 남아 있는데도 그 몫이 미배분이므로 이 안내가 거짓이 된다. **의무가입기간이 지난 ISA가 살아남은 경우에도 나가지 않는다**(`13.0.0`) — 연금 한도가 그대로 남아 있기 때문이다. 그 상태는 `unallocated_breakdown.reason_code`가 말한다 |
...
| `fund_use_horizon_excluded_from_amounts` | 자금 사용 시점은 배분 금액·세액공제액에 반영하지 않음. 순서와 경고에만 쓰임. **`12.0.0`부터 `fund_use_horizon`이 `within_isa_lock_in`이면 나가지 않는다** — 그 값에서는 이 진술이 거짓이기 때문이다(3.1절·D52 2번). **ISA가 살아남아도 나가지 않는다**(`13.0.0`) — 연금 두 계좌의 금액이 여전히 그 시점 때문에 0이다. 나머지 세 값에서는 그대로 참이고 그대로 나간다 |
...
| `all_accounts_have_early_exit_penalty` | `fund_use_horizon`이 `within_isa_lock_in` **이고, 배분 대상인 계좌가 하나 이상 있으며, 그 계좌 전부가 중도 불이익을 질 때**. **`13.0.0`부터 이 조건과 배분을 비우는 조건이 같은 사실을 본다**(3.1절 · D53 1번) — `12.0.0`까지는 이 자리에서만 잔여 의무가입기간을 보고 배분 쪽에서는 안 보았다. 뜻은 "어느 계좌도 중도 불이익을 피하지 못한다"이고, 화면은 배분 비교보다 이 사실을 앞세워야 한다 — **`12.0.0`부터 그 시점에는 전액이 미배분이므로 이 코드가 그 이유를 지는 자리다.** **불이익이 성립하지 않는 계좌가 하나라도 있으면 나가지 않는다**(예: ISA 의무가입기간이 이미 지난 사용자) — 성립할 수 없는 불이익을 주장하지 않기 위해서다(0.3절). **어느 계좌가 무엇을 지는가는 8.4절이 정하고 여기서 다시 적지 않는다**(8.0절). `11.0.0`까지는 「반환된 배분안이 하나도 빠짐없이 `warnings`를 가질 때」로 재었고, 배분이 사라지면서 그 방법이 아무것도 재지 못하게 됐다 |
...
- **`no_additional_tax_credit`** (`12.0.0`) — **더 넣어도 표시되는 세액공제액이 한 원도 늘지 않아 거기서 멈췄다.** **`13.0.0`에서 기준이 정확값에서 표시 금액으로 옮겨졌다**(D53 2번) — 세액 한도에 소수부가 있으면 정확값 기준은 **0.1원짜리 증가**를 「늘었다」로 세고, 그 0.1원은 `tax_credit`에도 `tax_credit_before_cap`에도 나타나지 않는다. 어느 표시 금액에도 안 나타나는 것은 얻은 것이 아니다. `retirement_pension`에만, 그리고 **기본안이 될 수 있는 안**(`max_tax_credit`·`isa_first`)에서만 나온다. 그 계좌에만 걸리는 이유는 조문이 두 연금계좌의 **중도인출**을 다르게 정하기 때문이고(`pension.withdrawal.midterm_restriction`), 그 규칙 id가 같은 `Allocation`의 `basis_rule_ids`에 실린다. **연금저축에는 결코 붙지 않는다** — 공제를 못 낳아도 인출이 자유롭고 이월 신청이 예정되므로 「아무것도 얻지 않는다」가 성립하지 않는다(5.12절). **화면이 이것을 「예산이 모자랍니다」나 「한도가 찼습니다」로 적으면 거짓이다** — 사용자가 돈을 더 넣으면 채워질 것처럼 읽힌다.
- **`fund_use_horizon`** (`12.0.0`) — **자금 사용 시점이 `within_isa_lock_in`이라 그 계좌를 비웠다.** 연금 두 계좌에는 언제나 붙고, **ISA에는 `fund_use_horizon_boundaries.isa_lock_in_years_remaining > 0`일 때만** 붙는다(`13.0.0` · D53 1번). 자격이 없는 계좌는 `not_eligible`이 이긴다. 근거 규칙은 8.10절과 같다. **잔여 기간이 0인 사용자의 ISA에 이 값이 붙으면 거짓이다** — 그 계좌를 막은 것은 시점이 아니라 예산이거나 납입 한도다.
...
- **`no_account_beneficial_within_fund_use_horizon`** — **자리는 있는데 그 시점에는 어느 계좌도 이롭지 않다.** `fund_use_horizon`이 `within_isa_lock_in`이고, **열려 있으면서 비워지지 않은 계좌가 하나도 없을 때**다(`13.0.0` · D53 1번 — 의무가입기간이 지난 ISA가 살아남으면 이 값이 아니라 `contribution_room_exhausted`다). 이때 `basis_rule_ids`에 `isa.account.requirements` · `isa.early_termination.clawback` · `pension.early_withdrawal.other_income_rate` · `tax.local.personal_income_surtax`가 함께 실린다 — **엔진이 그 규칙을 실제로 읽어서 이 판정을 만들고, 하나라도 읽지 못하면 `rule_missing`으로 멈춘다.**
...
### 8.11 연금 역산기의 코드 공간 (`14.1.0`, D77)

**표를 두지 않는다.** 8.0절이 코드 조건 표를 8.1~8.5절에만 두라고 정했고 `scripts/org/validate-code-definitions.mjs`가 그것을 기계로 강제한다. 8.6~8.10절과 같은 형태로 산문에 적는다.

**두 탭은 코드 목록을 나눠 쓴다.** 절세계좌 계산기의 안내·가정은 `NOTICE`·`ASSUMPTION`이고, 연금 역산기의 것은 **`REVERSE_NOTICE`·`REVERSE_ASSUMPTION`**이다(둘 다 `src/engine/constants.mjs`).
...
- `reverse_return_rate_not_supplied` (info) — `profile.average_annual_return_rate`가 `null`이라 **계좌별 월 납입 시나리오 블록이 통째로 없다**(`contribution_scenario: null`). **0으로 계산하지 않았다는 뜻이다** — 「효과가 없다」가 아니라 「묻지 않았다」이고, 화면이 수익률을 제안하거나 미리 채우면 안 된다(D77 판정 1).
- `reverse_accumulation_period_not_positive` (info) — `as_of_date`부터 개시 시점까지 남은 개월수가 0 이하라 적립기 계산이 성립하지 않는다. 법정 사실 블록과 전략 비교는 그대로 나간다. `params.accumulation_months`.
- `reverse_target_already_funded` (info) — 현재 연금계좌 잔액(과 퇴직금 재원)만으로 필요 개시 평가액에 닿아 월 납입액이 0이다. **오류가 아니라 결과다.** `params.target_krw`.
- `reverse_annual_cap_binds_before_balance` (warning) — AC-R12. 수령 기간이 짧아 **연금수령한도가 잔액 요건보다 먼저 문다.** `params.payout_years` · `params.min_payout_years_without_cap_binding` · `params.annual_cap_floor_krw` · `params.remaining_balance_floor_krw`. **두 값이 같으면 나가지 않는다** — 없는 제약을 말하지 않기 위해서다.
- `reverse_exceeds_statutory_contribution_ceiling` (warning) — AC-R17. 역산한 월 납입액이 세 계좌 법정 납입 상한의 합계를 넘는다. `params.required_monthly_krw` · `params.ceiling_monthly_krw` · `params.unallocatable_monthly_krw`. **정확히 같으면 나가지 않는다.**
- `reverse_public_pension_not_supplied` (info) — `profile.public_pension.plan`이 `"yes"`가 아니라 목표 월액 전부를 사적연금 몫으로 보았다. `params.plan`.
- `reverse_public_pension_covers_target` (info) — 국민연금 예상 월액이 목표 월액 이상이라 사적연금이 채울 몫이 0이다. **음수를 내지 않고 0과 사실로 낸다.** `params.public_monthly_krw` · `params.target_monthly_krw`.
- `reverse_public_pension_start_age_not_in_ruleset` (info) — 공적연금 개시 연령을 정한 규칙이 룰셋에 없어 **개시 시점 사이의 공백 구간을 계산하지 않았다.** 룰셋에 없으면 없는 것이고 엔진이 연령을 지어내지 않는다.
...
**가정 코드**(`REVERSE_ASSUMPTION`) — 응답의 `assumptions[]`로 나가고 형태는 4.3절과 같되 `applies_to_scenarios`가 없다(역산기는 시나리오를 하나만 돌린다).

- `reverse_zero_growth_for_statutory_cap` — D77 판정 1. 연금수령한도 판정과 필요 최소 평가액을 **무성장(수익률 0)** 기준으로 고정했다. **이 고정이 층 4를 열지 않는 방법이다.** 룰셋 `pension.withdrawal.annual_cap.inverse_use.assumption_stated_once`가 근거다.
- `reverse_level_annual_withdrawal_assumed` — 수령 기간 동안 **매년 같은 금액**을 꺼낸다고 보았다. 이 가정 위에서만 「필요 최소 평가액」이 하나의 수가 된다.
- `reverse_first_withdrawal_year_index_assumed` — 연금수령연차의 **기산연차를 1로** 보았다. `params.withdrawal_year_index`. **기산이 다른 집단이 있고 그 집단의 첫해 한도는 두 배다**(시행령 §40조의2④ · 정답지 GC-P5b). 계좌 개설일을 입력으로 받지 않으므로 두 집단을 구별할 수 없고, 한쪽을 단정하는 대신 **어느 쪽을 보았는지를 값으로 낸다.** 화면은 이 조건을 금액과 같은 자리에 적어야 한다.
- `reverse_monthly_compounding` — 적립기 복리를 **월 복리**로 통일했다. `params.periods_per_year`. 세법이 복리·단리도 주기도 정하지 않으며, 납입이 월 단위이므로 이자 주기를 월로 맞춘 것은 **이 조직의 관행**이다.
- `reverse_return_rate_user_supplied` — 계좌별 월 납입액은 **사용자가 제시한 수익률** 위의 계산이다. `params.annual_return_rate`. **이 서비스는 수익률을 제시하지 않는다.**
...
**필드 전용 열거형** — 안내·가정 어느 목록에도 속하지 않는다.
...
- `contract_held_withdrawal.reason_code` — `"isa_tenure_missing"` / `"after_min_contract_years_not_settled"` / `"cumulative_contribution_missing"` / `null`. **`deemed_terminated`가 `null`인 이유를 가른다.** 그중 `"after_min_contract_years_not_settled"`는 **조문이 규율하지 않아** 참도 거짓도 내지 않는 자리다.
...
## 12. 세 번째 진입점 — 연금 역산기 `computePensionReverse` (`14.1.0`, D77)

**번호가 9 다음에 12인 것은 의도다.** 10·11절은 이 문서를 여는 지침과 부록이고, 그 뒤에 새 계약을 붙이면 계약이 부록 아래로 들어간다. 소절 번호는 다른 문서와 `src/web`의 주석이 가리키므로 **하나도 다시 매기지 않는다**(0절 머리의 규약).

```
computePensionReverse(request: PensionReverseRequest, rulesets: RulesetBundle): PensionReverseResponse
```

**1절의 성질을 그대로 진다** — 순수 함수, 예외 없음, 문구 없음, 룰셋은 인자로 주입. **다만 「현재 시각을 읽지 않는다」를 지키기 위해 오늘을 인자로 받는다**(`as_of_date`). 적립 기간이 오늘의 함수이므로 시계를 읽으면 같은 입력이 다른 답을 낸다.

**첫 탭과 상태를 공유하지 않는다.** `compute`의 요청·응답과 겹치는 필드가 하나도 없고, 두 진입점이 함께 쓰는 것은 룰셋 접근·원 미만 규약·날짜 산술뿐이다.

### 12.1 `PensionReverseRequest` — 최상위

| 필드 | 자료형 | 단위 | 필수 | 설명 / null일 때 |
|---|---|---|---|---|
| `schema_version` | string | — | 필수 | major가 다르면 `schema_version_mismatch`. 0절 머리가 정의 자리다 |
| `tax_year` | integer | 년 | 필수 | 읽을 확정 룰셋을 고른다. **개정예고 룰셋은 읽지 않는다** — 역산기는 확정 시나리오만 돌린다 |
| `as_of_date` | string | `YYYY-MM-DD` | 필수 | **오늘.** 적립 기간의 시작점이다. 화면이 채운다. 달력에 없는 날짜면 `invalid_date` |
| `profile` | ReverseProfile | — | 필수 | 12.2절 |
| `accounts` | ReverseAccounts | — | 필수 | 12.3절 |

### 12.2 `ReverseProfile`

| 필드 | 자료형 | 단위 | 필수 | 설명 / null일 때 |
|---|---|---|---|---|
| `birth_date` | string | `YYYY-MM-DD` | **필수** | 생년월일. `as_of_date`보다 뒤면 `out_of_range`. 개시 시점의 만 나이와 연령별 원천징수세율 구간을 정한다 |
| `target_monthly_income_krw` | integer | 원/월 | **필수** | 원하는 연금 수령액. **0 초과**(0이면 `out_of_range`). **오늘 화폐 기준이다** |
| `annuity_start` | `{ kind: "age", age_years: integer }` \| `{ kind: "year", year: integer }` | 년 / 연도 | **필수** | 연금 개시. **나이로도 연도로도 답할 수 있고 둘은 같은 날짜로 환산된다** — 그 해의 생일이다(가정 `reverse_annuity_start_date_derived_from_birthday`). `kind`가 목록 밖이면 `invalid_enum`. 그 날의 만 나이가 룰셋의 최소 개시 연령 미만이면 `annuity_start_below_minimum_age`이고 **응답에 어떤 블록도 없다** |
| `payout_years` | integer | 년 | **필수** | 연금 필요 기간. 1 이상. **확정기간형 전제다** — 종신형은 이 계약 밖이다(9.3절) |
| `average_annual_return_rate` | number \| null | 비율(연) | 선택 | 적립기 평균 수익률. 0 이상. **`null`이면 계좌별 월 납입 시나리오 블록이 통째로 없다**(`contribution_scenario: null`). **0을 기본값으로 만들지 않는다** — `0`은 사용자가 준 값일 때만 계산된다. 화면은 이 칸에 기본값·자리표시자·힌트를 두지 않는다(D77 판정 1) |
| `public_pension.plan` | `"yes"` \| `"no"` \| `"unknown"` | — | 선택 | 국민연금 등 공적연금 수령 계획. 기본은 `"unknown"` |
| `public_pension.expected_monthly_krw` | integer \| null | 원/월 | 위가 `"yes"`일 때 필수 | 예상 월액. **`"yes"`가 아닌데 값이 실려 오면 `invalid_enum`** — 둘 중 무엇이 사용자의 답인지 엔진이 고르지 않는다. 이 서비스는 국민연금 급여액을 스스로 계산하지 않는다 |
| `other_income.state` | `"known"` \| `"unknown"` | — | 선택 | 연금 수령 시기의 연금 외 종합소득을 아는가. 기본은 `"unknown"` |
| `other_income.annual_krw` | integer \| null | 원/연 | 위가 `"known"`일 때 필수 | **`"known"`이 아니면 「문턱 초과 감수」의 유불리를 판정하지 않는다**(AC-R10). `"known"`이 아닌데 값이 실려 오면 `invalid_enum` |
| `deferred_retirement.present` | boolean \| null | — | 선택 | 퇴직금(이연퇴직소득) 재원 유무. `null`은 「아니오」와 같이 다룬다 |
| `deferred_retirement.amount_krw` | integer \| null | 원 | 위가 `true`일 때 필수 | **이 재원은 1,500만원 문턱을 한 원도 쓰지 않는다.** `true`가 아닌데 값이 실려 오면 `invalid_enum` |

### 12.3 `ReverseAccounts`

| 필드 | 자료형 | 단위 | 필수 | 설명 / null일 때 |
|---|---|---|---|---|
| `annuity_savings.balance_krw` | integer | 원 | **필수** | 연금저축계좌 현재 평가액. 계좌가 없으면 0 |
...
```
{ ok: true,  schema_version, echo, statutory_facts, contribution_scenario,
  contribution_scenario_absent_reason_code, payout_strategies, assumptions, notices, legal_basis }
{ ok: false, schema_version, errors }
```

| 필드 | 자료형 | 설명 |
|---|---|---|
| `echo` | ReverseEcho | 12.5절 |
| `statutory_facts` | StatutoryFacts | 12.6절. **`ok: true`이면 언제나 있다** — 수익률이 없어도 있다 |
| `contribution_scenario` | ContributionScenario \| null | 12.7절. **`null`이면 그 블록이 없다는 뜻이고, 「0원」이 아니다** |
| `contribution_scenario_absent_reason_code` | string \| null | 8.11절의 열거형. 블록이 있으면 `null` |
| `payout_strategies` | PayoutStrategy[] | 12.8절. 길이는 2 또는 3 |
| `assumptions` | Assumption[] | 4.3절과 같되 `applies_to_scenarios`가 없다. 코드는 8.11절 |
| `notices` | Notice[] | 7.2절과 같은 형태. 코드는 8.11절과 8.2절 |
| `legal_basis` | LegalBasisEntry[] | 5.7절과 같은 타입. **읽은 규칙만 실린다** |
| `errors` | EngineError[] | 8.1절과 같은 코드 체계. **발견한 것을 전부 담는다** |

### 12.5 `ReverseEcho`

| 필드 | 자료형 | 단위 | 설명 |
|---|---|---|---|
| `as_of_date` | string | `YYYY-MM-DD` | 요청 그대로 |
| `annuity_start_date` | string | `YYYY-MM-DD` | 엔진이 환산한 개시 시점. **나이로 답했든 연도로 답했든 이 값 하나로 모인다** |
| `annuity_start_age_years` | integer | 년 | 그 날의 만 나이. 연령별 원천징수세율이 이 값에서 나온다 |
| `accumulation_months` | integer | 월 | `as_of_date`부터 개시 시점까지의 **완전한** 개월수. 0 이하일 수 있고 그때 블록 ②가 없다 |
| `payout_years` | integer | 년 | 요청 그대로 |
| `average_annual_return_rate` | number \| null | 비율 | 요청 그대로. **화면은 이 값을 조건절에 그대로 넣는다**(AC-R15) |
| `return_rate_affects` | `{ statutory_facts: false, payout_strategies: false, contribution_scenario: true }` | — | **고정 객체다.** 층 4를 열지 않았다는 선언이고, 앞 둘이 `false`인 것이 AC-R16이다. 단위 시험이 두 응답을 맞대어 이 선언이 참인지 확인한다 |

### 12.6 `StatutoryFacts` — 블록 ①, 가정 0

**이 블록의 어느 값도 `average_annual_return_rate`를 읽지 않는다.**

| 필드 | 자료형 | 단위 | 설명 |
|---|---|---|---|
| `ruleset.tax_year` · `ruleset.files` | integer · string[] | — | 어느 룰셋으로 계산했는가 |
| `target.target_monthly_krw` · `target_annual_krw` | integer | 원/월 · 원/연 | 요청의 목표와 그 12배 |
| `target.public_pension_monthly_krw` | integer \| null | 원/월 | `plan`이 `"yes"`가 아니면 `null` |
| `target.private_pension_required_monthly_krw` · `_annual_krw` | integer | 원/월 · 원/연 | **사적연금이 채울 몫 = 목표 − 국민연금.** 음수면 0이다 |
| `target.public_pension_covers_target` | boolean \| null | — | 국민연금만으로 목표가 채워지는가. 미입력이면 `null` |
| `target.public_pension_start_gap_evaluated` | boolean | — | **언제나 `false`.** 공적연금 개시 연령 규칙이 룰셋에 없어 공백 구간을 계산하지 않았다 |
| `minimum_start_balance.required_krw` | integer | 원 | **개시 시점 필요 최소 평가액.** 두 하한의 큰 쪽이고 원 미만은 올림이다 |
| `minimum_start_balance.first_year_floor_krw` | integer | 원 | 첫 해만 놓고 본 하한. **어떤 가정도 없이 조문의 계수 둘과 나눗셈 하나에서 나온다** |
| `minimum_start_balance.annual_cap_floor_krw` | integer | 원 | 연금수령한도가 요구하는 하한 |
| `minimum_start_balance.remaining_balance_floor_krw` | integer | 원 | 잔액이 요구하는 하한(= 기간 × 연 수령액). **세법이 아니라 산수다** |
| `minimum_start_balance.binding_code` | string | — | 8.11절 |
| `minimum_start_balance.cap_binds_before_balance` | boolean | — | AC-R12의 판정 |
| `minimum_start_balance.last_year_the_cap_binds` | integer | 년 | 한도가 마지막으로 무는 연차 |
| `minimum_start_balance.cap_ceases_at_year_index` | integer | 년 | 이 연차부터 산식을 적용하지 아니한다. **룰셋에서 읽은 값이다** |
| `minimum_start_balance.min_payout_years_without_cap_binding` | integer | 년 | 이 기간 이상이면 한도가 잔액보다 먼저 물지 않는다. **조문에 적혀 있는 수가 아니라 계수에서 나오는 산술이다** |
| `minimum_start_balance.withdrawal_year_index` | integer | — | **언제나 1.** 기산연차 가정이 응답에 같이 실린다 |
| `minimum_start_balance.basis_rule_ids` | string[] | — | 조문 인용의 근거(AC-R11) |
| `target_feasibility.holds_if_start_balance_at_least_krw` | integer | 원 | 「이 목표는 개시 시점 평가액이 이 값 이상이면 성립한다」 |
| `target_feasibility.current_pension_balance_krw` · `deferred_retirement_krw` | integer | 원 | 지금 가진 것 |
| `target_feasibility.max_first_year_withdrawal_at_current_balance_krw` | integer \| null | 원/연 | **지금 잔액만으로 첫 해에 연금수령할 수 있는 최대 금액.** 가정이 하나도 없다 |
...
**이 블록의 모든 금액에는 조건절이 붙는다**(AC-R15). 조건절의 재료가 `condition_clause`다.

| 필드 | 자료형 | 단위 | 설명 |
|---|---|---|---|
| `condition_clause.annual_return_rate` | number | 비율(연) | **사용자가 준 값 그대로** |
| `condition_clause.is_user_supplied` | boolean | — | **언제나 `true`.** 이 서비스는 수익률을 제시하지 않는다 |
...
**셋 중 어느 것에도 「선택하세요」·「추천합니다」에 해당하는 값이 없다**(AC-R19). 엔진이 내는 것은 각 경로의 법정 사실과, 입력이 있을 때만 나오는 조건부 유불리뿐이다. **세액은 전부 첫 해 기준이다.**

**① `within_threshold`** — `private_pension_annual_krw`(문턱과 목표 중 작은 쪽) · `uncovered_annual_krw`(문턱으로 못 덮는 몫) · `first_year_withholding{rate, income_tax_krw, local_tax_krw, total_krw}` · `settles_with_withholding: true` · `first_year_after_tax_krw` · `basis_rule_ids`.

**② `exceed_threshold`** — `private_pension_annual_krw` · `elective_opens` · `elective_base_krw`(**초과분이 아니라 전액이다**) · `first_year_withholding` · `comprehensive`(제1호) · `separate`(제2호) · `comparison_code` · `lower_option_code` · `first_year_after_tax_krw` · `basis_rule_ids`.

- `comprehensive` — `total_pension_krw`(공적 + 사적) · `deduction_krw` · `pension_income_krw` · `tax_base_krw` · `income_tax_krw` · `local_tax_krw` · `total_krw`. **연금소득공제는 공적연금과 합산한 총연금액에 한 번 걸린다.**
...
- **연차별 잔액 추이와 연차별 세액.** 완전한 다년 시뮬레이션은 열지 않는다(AC-R25).
- **수령기 수익률을 반영한 한도 판정.** 층 4다(D77 판정 1).
...
- **한도가 0일 때 `priority_basis.objective_degenerate`를 읽는다.** 배분안의 수는 그대로이지만(**1개 이상 4개 이하**) 세액공제액으로는 갈리지 않는다. 순위를 절세액 순으로 설명하면 없는 근거를 말하게 된다(5.12절).
- **`contribution_carryover_available`이 `true`면 "넣은 돈이 사라진다"고 쓰지 않는다.** 소멸하는 것은 그해의 세액공제액이고 납입액은 **신청을 통해** 이후 과세기간으로 넘길 수 있다.
- **이월·전환 안내를 `applied`에 매달지 않는다**(`14.0.0` · D54). 「다음 해로 넘길 수 있습니다」·「전환 신청」류의 문장이 서는 자리는 **`contribution_carryover_available`** 하나다. 두 칸은 다른 물음에 답하고 **`applied: true`이면서 그 칸이 `false`인 사용자가 실재한다** — 그 사용자에게는 **넘길 금액이 한 원도 없다.** 없는 절차로 보내는 것이 이 금지가 막는 것이다(5.5절).
...
## 11. 부록 — 버전 이력과 major 판정의 근거

**여기 있는 것은 경위다.** 지금 유효한 것은 0절(색인)과 3~10절(정의 자리)이고, **이 절의 서술이 정의 자리와 어긋나면 정의 자리가 이긴다**(8.0절). 각 소절은 **적힌 시점의 사정**을 담는다.

**그런데도 지우지 않는다.** major 판정의 근거는 다음에 같은 판단을 할 사람에게 **그대로** 쓰인다 — 이 저장소에서 `0.1`의 근거가 `8.0.0`·`9.0.0`·`11.0.0`에서 세 번 다시 인용됐고, `0.9`가 minor를 고른 근거는 `0.11`이 major를 고를 때 저울의 반대편이 됐다. **접는 것은 되고 자르는 것은 안 된다.**

**소절 번호는 지금까지 붙은 그대로다.** 다른 문서와 `src/web`의 주석이 「계약 0.12절」처럼 이 번호로 가리키고 있어 번호를 다시 매기면 그 참조가 전부 끊긴다. 그래서 **위치만 옮기고 번호는 손대지 않았다.**
...
| `13.0.0` | D53 | **D53 — 관리자가 판정해 놓고 계약에 옮기지 않은 것 하나와, 스스로 거짓이라고 확인한 문장 하나.** ① **의무가입기간이 이미 지난 ISA는 배분을 유지한다**(D52 후속 판정이 `12.0.0`에 반영되지 않았다). 같은 판정을 8.5절이 이미 하고 있었고 배분 쪽만 안 보고 있었다. ② **트림의 기준이 정확값에서 표시되는 세액공제액으로 옮겨진다** — 한도에 소수부가 있으면서 무는 모든 좌표에서 무의미한 IRP가 몇 원 남고 있었다. 그 결과 `tax_liability_cap.applied`가 **트림 전 금액**에 대고 재는 값이 된다(5.5절). ③ **표시 공제액이 같고 IRP만 더 묶는 안은 `plans`에서 뺀다** — 「목적」을 이름이 아니라 값으로 잰다(6.2.1절). **같은 요청이 다른 금액을 내고 `plans` 배열의 구성이 달라진다. 왜 major인지는 0.21절** |
...
| `8.2.0` | D38 | **D38 — 헤드라인이 세액공제만의 것이 아니게 됐다.** `Plan`에 `headline_composite_total`(합계의 두 끝·점/구간 코드·가정 성분 유무)이 붙고, `AssumptionBasedIsaEstimate`에 `axis_ceilings`(비과세 축의 상한과 그 상한이 재는 **기간**, 나머지 두 축의 상한 유무)가 붙는다. **기존 필드는 하나도 바뀌지 않고 금액도 한 원도 움직이지 않는다** — 새 값은 전부 새 자리에 실린다. 함께 **틀린 수 하나를 고쳤다**: 연 환산 과대율의 「최대 1.75배」는 거꾸로였고 1.75는 **하한**이다(0.16절). **왜 minor인지는 0.15절** |
| `8.1.0` | D36 · D37 | **D36 — 축을 둘로 가른 화면이 그려지려면 계약에 없던 값 둘이 필요했다.** `ScenarioResult`에 `pension_credit_ceiling`(확정 축의 최댓값)과 `pension_withdrawal_tax_reference`(연금 저율과세 세율표, **금액 없음**)가 붙는다. `AssumptionBasedIsaEstimate`에 `axis_breakdown_bound_code`·`rate_gap_axis_zero_reason_code`가 붙고, 안내 코드 `isa_rate_gap_axis_zero_because_within_tax_free_limit`이 늘었다. **기존 필드는 하나도 바뀌지 않고 금액도 한 원도 움직이지 않는다.** **왜 minor인지는 0.14절** |
| `8.0.0` | — (소유자 신고 후속, 서술 정정) | **`7.0.0`이 적은 월 환산 이탈 범위가 산술로 틀렸다.** `monthly_annualized_krw − annual_krw`의 범위를 `±(개월수 − 1)`로 적었으나 실제는 **−(개월수 − 1) 이상 (갈래 수 − 1) × (개월수 − 1) 이하**다 — 한 갈래가 조정을 둘·셋 받는 좌표가 실재한다. **응답은 한 원도 바뀌지 않는다. 필드도 요청 형태도 그대로다.** 그런데도 major인 이유는 **계약이 준 범위 보장을 거두기 때문**이고, 규약과 0.1절이 그것을 major로 정한다. **왜 major인지는 0.13절** |
| `7.0.0` | — (소유자 신고) | **소유자 신고 — 월 배분 총액이 1원 모자란다.** 계좌별 월 금액이 더 이상 `floor(연 ÷ 개월수)`가 아니다. 내림으로 버려지던 잔차를 **납입 한도 여유가 남은 갈래에 얹어** 월 표시 금액의 합이 월 납입 여력과 정확히 같아진다. `Allocation`에 `monthly_annualized_krw`·`monthly_rounding_adjustment_krw`가, `Plan`에 `monthly_unassigned_krw`·`monthly_unassigned_reason_code`·`unallocated_monthly_rounding_adjustment_krw`가 붙는다. **연간 금액·한도·세액공제액은 한 원도 움직이지 않는다.** **왜 major인지는 0.12절** |
| `6.0.0` | D32 | **D32 — 소유자가 기본안의 배분을 바꿨다.** 기본안의 충당 순서에 「연금계좌를 **납입** 한도까지」 단계가 붙어 **네 안이 전부 납입 한도까지 채운다.** `plan_id` 열거형에서 `pension_contribution_limit_fill`이 빠지고 `pension_contribution_before_isa`가 그 자리에 들어온다. `limited_by`의 `credit_limit`이 **사라진다.** `pension_contribution_without_credit` 효과가 기본안에서도 나온다. **왜 major인지는 0.11절** |
| `5.1.0` | D28 · D29 · D31 | **D28·D29·D31 — 수익률을 받는다.** 요청에 `profile.isa_return_assumption`(선택)과 `options.assumption_based_isa_estimate`(선택)가 붙고, 응답에 `Plan.assumption_based_isa_estimate` · `echo.isa_return_assumption` · `echo.isa_return_affects`가 추가된다. 안내 코드 6건·가정 코드 7건이 늘었다. **왜 minor인지는 0.9절** |
| `5.0.0` | D26 · D27 | **D26·D27.** 공제율 판정 축을 두 물음으로 나눴다 — `profile.has_non_wage_global_income_current_year`가 **필수로** 들어오고 `profile.current_year_global_income_krw`가 선택으로 붙는다. 배분안이 셋에서 **넷**으로 늘고(`pension_contribution_limit_fill`), `Plan`에 `unallocated_breakdown`·`pension_combined_credit_remaining_after_plan_krw`가 추가되며, `NonQuantifiedEffect`에 `facts`·`headroom_shared_with`가, `LegalBasisEntry`에 `uncertainty_notes`가 붙는다. **왜 major인지는 0.6절** |
| `4.0.0` | D21 · D22 | **세액 한도·연금수령 개시·개시 가능 시점·퇴직급여 입금.** `tax-domain` 6차 조사의 확정 규칙 6건을 엔진이 읽는다. 요청에 `profile.birth_date`·`profile.prior_year_tax`·`accounts.*.annuity_start_status`가 **필수로** 들어오고 `profile.age_years`가 **사라진다.** 응답에 `pension_credit_tax_liability_cap`·`pension_withdrawal_start`·`DeterministicBenefit`의 자르기 전후 금액이 추가된다. **왜 major인지는 0.5절** |
| `3.3.1` | D18 | **D18 — 조건이 두 곳에 적혀 어긋난 것의 정정, 그리고 재발 구조를 없앤 개정.** 8.4절 맺음 문장이 `isa_lock_in_already_elapsed`의 조건을 8.2절과 다르게 적고 있어 좁혀서 8.2절에 맞췄다. 조건을 **한 곳에만** 적게 하는 규약을 8.0절로 세우고 `engine-design.md` 3.2·3.3절의 중복 서술을 참조로 바꿨다. 확정 시나리오의 `tie_break` 값을 5.6절에 명시했다. **엔진 동작은 한 줄도 바뀌지 않았다** — 서술만 정리했으므로 patch다 |
| `3.3.0` | 게이트 4 후속 | **게이트 4 후속 — 세제상 동점일 때의 순서 원칙.** `max_tax_credit`·`isa_first`가 동점 구간에서 인출이 자유로운 연금계좌를 먼저 채운다(0.4절). `PriorityBasis`에 `tie_break`를 추가했다(5.6절). 필드 추가뿐이고 `max_tax_credit`이 세액공제를 최대화한다는 보장은 그대로여서 minor다 |
| `3.2.0` | 4단계 교차검증 M3 | **4단계 2차 교차검증 M3의 정정.** `all_accounts_have_early_exit_penalty`의 발생 조건을 8.4절의 경고와 맞췄다(0.3절, 8.5절). `3.1.0`과 같은 이유로 minor다 — **없어야 할 안내가 사라지는 방향**이다 |
| `3.1.0` | 4단계 교차검증 M2 | **4단계 교차검증 M2의 정정.** `early_termination_clawback_isa` 경고 조건에 "의무가입기간이 남아 있을 것"을 추가했다(0.2절, 8.4절). 안내 코드 `isa_lock_in_already_elapsed`를 추가했다(8.2절). 필드 추가·제거가 없고 **없어야 할 경고가 사라지는 방향**이라 기존 소비자가 깨지지 않으므로 minor다 |
| `3.0.0` | — (3단계 구현 중 발견) | **3단계 구현 중 발견된 계약 오류의 정정.** `Plan.delta_vs_baseline_krw`의 "0 이하" 제약을 제거했다(0.1절). `AccountLimit`에 `contribution_limit_shared_with`·`credit_limit_shared_with`를 추가해 계좌 간 공유 한도를 구조로 드러냈다(5.3절) |
| `2.1.0` | 게이트 2 마감 판단 | **게이트 2 마감 판단.** 경계값 전용 진입점 `computeFundUseHorizonBoundaries` 추가(9절). 기존 진입점과 타입은 그대로이므로 minor다 — `2.0.0`에 맞춘 목은 계속 동작한다 |
| `2.0.0` | D10 | **게이트 2 D10.** 입력 `profile.risk_profile`을 제거하고 `profile.fund_use_horizon`으로 대체. `Plan.warnings`, `ScenarioResult.fund_use_horizon_boundaries` 추가. `echo.risk_profile*`을 `echo.fund_use_horizon*`으로 교체 |
| `1.0.0` | 게이트 2 제출본 | 최초 계약 (게이트 2 제출본) |

### 11.B 근거 색인 — 어느 소절이 무엇을 판정했나

| 소절 | 무엇을 판정했나 | 버전 |
|---|---|---|
| 0.1 | `delta_vs_baseline_krw` 제약 제거 — 부호 보장을 거두는 것이 왜 major인가 | `3.0.0` |
| 0.2 | ISA 추징 경고 조건을 좁힌 경위 | `3.1.0` (M2) |
| 0.3 | 비교 안내를 경고와 묶은 경위 | `3.2.0` (M3) |
| 0.4 | 세제상 동점일 때의 순서 원칙 — 인출이 자유로운 계좌를 먼저 | `3.3.0` |
| 0.5 | 왜 `4.0.0`(major)인가 — 새 입력을 선택 필드로 두지 않은 이유 | `4.0.0` |
| 0.6 | 왜 `5.0.0`(major)인가 — 세 가지가 겹쳤다 | `5.0.0` |
| 0.7 | 공제율 판정 축을 두 물음으로 나눈 경위 (D27) | `5.0.0` |
| 0.8 | ISA 유형 교차확인 — 결론을 낼 수 있을 때만 낸다 | `5.0.0` |
| 0.9 | 왜 `5.1.0`(minor)인가 — 조용히 틀릴 길이 없다 | `5.1.0` |
| 0.10 | 수익률을 들이면서 지킨 선 셋 (D28 · D31) | `5.1.0` |
| 0.11 | **(a)** 왜 `6.0.0`(major)인가 — 조용히 틀릴 길이 셋이다 | `6.0.0` |
| 0.11 | **(b)** 되돌리는 길 — 계산과 입력은 두고 표시만 끈다 (D31) | `5.1.0` |
| 0.12 | 월 환산 잔차를 어디로 보내는가, 그리고 왜 `7.0.0`(major)인가 | `7.0.0` |
| 0.13 | 왜 `8.0.0`(major)인가 — 값이 하나도 안 바뀌는데 major인 이유 | `8.0.0` |
| 0.14 | 왜 `8.1.0`(minor)인가 — 확정 축의 최댓값을 무엇으로 정했나 | `8.1.0` |
| 0.15 | 왜 `8.2.0`(minor)인가 — 합계를 어떻게 거짓이 아니게 만들었나 | `8.2.0` |
| 0.16 | 우리가 적어 온 과대율이 거꾸로였다 — 1.75는 상한이 아니라 하한 | `8.2.0` |
...
| 0.22 | 왜 `14.0.0`(major)인가 — 금액이 한 원도 안 바뀌는데 major인 이유 | `14.0.0` |
| 0.21 | 왜 `13.0.0`(major)인가 — 보장 셋을 거두고 `plans`의 구성을 바꾼다 | `13.0.0` |
...
### 0.22 왜 `14.0.0`(major)인가 — 금액이 한 원도 안 바뀌는데 major인 이유

**요청은 한 글자도 바뀌지 않는다. 새 입력도, 새 필드도 없다. 금액도 한 원도 움직이지 않는다.** 뒤집히는 것은 참·거짓 한 칸과 그것에 딸린 셋뿐이다. 그런데도 major인 이유가 셋이다.

1. **기존 필드의 의미가 바뀐다.** 규약(0.D절)이 그것을 major로 정한다. `13.0.0`까지 이 칸은 「`applied`와 같은 값」이었고 계약이 그렇게 **적어 두었다**(「`applied`와 같이 움직인다」). 그 문장을 읽고 한쪽만 읽어도 된다고 판단한 소비자는 계약을 어긴 적이 없는데 이제 다른 값을 받는다. 0.1절이 세운 근거가 그대로 걸린다.
2. **응답 값이 실제로 바뀌는 사용자가 있다.** 「한도에 소수부가 있으면서 그 소수부만큼만 무는」 좌표 전부다. 그 사용자의 응답에서 `contribution_carryover_available`이 `true` → `false`가 되고, `carryover_shares_future_year_credit_limit`·`carryover_requires_application`이 값 → `null`이 되며, `basis_rule_ids`에서 규칙 하나가 빠진다. **확인된 좌표: 총급여 24,795,208 · 월 2,666,667**(`src/engine/tax-liability-cap.test.mjs`가 못 박았다).
3. **조용한 실패가 시끄러운 실패보다 비싸다**(0.1절·0.5절과 같은 근거). **이 변경은 금액을 하나도 바꾸지 않으므로 금액을 보는 어떤 검사에도 걸리지 않는다.** minor로 내면 목은 계속 돌면서 이 칸을 옛 뜻으로 읽고, 화면은 **넘길 금액이 한 원도 없는 사용자**에게 「다음 해로 넘길 수 있습니다」를 계속 적는다. major로 올리면 `src/web`의 목이 `schema_version_mismatch`로 즉시 멈추고, **멈추는 시점이 마침 화면이 이 칸을 다시 읽어야 하는 시점**이다.

**채택하지 않은 대안 둘.**

- **정확값으로 재기(`applied`를 그대로 두기).** 기각한다. `13.0.0`이 트림 기준을 표시 금액으로 옮겨 막은 형태가 **여기서 그대로 되살아난다** — 어느 표시 금액에도 나타나지 않는 끝수를 「있는 것」으로 세는 것이다. GC-32d의 정확값 초과분 0.1원에 대응하는 납입액은 **0.67원**이고, 그것을 「가장 먼저 인출하여 다시 납입한 것으로 본다」는 처분은 실행될 수 없다.
- **`reduced_total_krw > 0`으로 재기.** 이 룰셋에서는 `reduced_income_tax_krw > 0`과 같은 값을 낸다(지방소득세분은 소득세분에서 산출되므로 소득세분이 안 줄면 지방세분도 안 준다). **소득세분을 고른 이유는 대응 관계다** — 전환의 대상은 **납입액**이고, 그 납입액에 대응하는 것은 공제율을 곱해 나온 **소득세분** 공제액이다. 지방소득세분은 그 소득세분의 함수라 자를 새로 세울 근거가 없다.

### 0.21 왜 `13.0.0`(major)인가 — 보장 셋을 거두고 `plans`의 구성을 바꾼다

**요청은 한 글자도 바뀌지 않는다. 새 입력도 없다.** 그런데도 major인 이유가 넷이다.

1. **`12.0.0`이 명시로 적은 보장이 거짓이 된다.** 3.1절이 「`within_isa_lock_in`에서는 **모든** 배분안의 세 계좌 금액이 0이고 예산 전액이 미배분이다」라고 적었고, 화면이 그 문장을 읽고 「전액 미배분 화면」을 만들 수 있었다. 이제 그 사용자 중 일부는 **ISA에 예산 전액이 들어간 응답**을 받는다. 0.1절이 세운 근거가 그대로 걸린다 — **소비자가 가진 것은 문서뿐이고, 문서가 보장한 것을 거두면 그것은 계약 변경이다.**
2. **같은 요청이 다른 금액을 낸다.** 두 갈래다 — 의무가입기간이 지난 ISA 보유자의 배분이 0에서 예산까지 오르고, 세액 한도에 **소수부가 있는** 사용자의 IRP가 몇 원 내려가면서 `tax_credit_before_cap`과 `threshold_income_tax_krw`가 따라 내려간다. 뒤엣것은 금액이 작지만 **그 값을 나란히 놓고 뺄셈하는 화면**에 그대로 나간다.
3. **`plans` 배열의 길이와 구성이 달라진다**(6.2.1절). 벡터가 다른데도 빠지는 안이 생겼고, 그 판정은 이름이 아니라 값이 한다. 배열 길이를 고정으로 가정하거나 특정 `plan_id`가 반드시 있다고 가정한 화면은 **조용히** 깨진다.
4. **`applied`가 대는 상대가 바뀐다**(5.5절). `12.0.0`까지 「반환된 배분의 자르기 전 금액 대 한도」였고 이제 「**트림 전** 금액 대 한도」다. 그 결과 **`applied: true`이면서 `reduced_total_krw: 0`인 자리가 새로 생긴다.** 두 값이 같이 움직인다고 믿은 화면은 「잘렸습니다」를 쓰면서 0원을 보여 준다 — `11.0.0`이 이미 그 어긋남을 열었고 D49가 화면이 볼 곳을 지정했으므로 **이번에는 그 지정이 지켜지는지가 관건이다.**

**채택하지 않은 대안 둘.**

- **`applied`를 반환된 배분 그대로 두기(트림 전을 안 보기).** 기각한다. 그러면 **끝수가 있는 한도가 무는 모든 사용자**에게 `applied: false`가 나가고, 화면은 한도 때문에 IRP가 잘린 사람에게 「한도에 걸리지 않았습니다」를 적는다. 정답지가 GC-32d에서 같은 형태를 이미 금지했다 — 「트림 후 값이 한도와 같아졌다고 해서 한도가 안 물었다고 적으면 원인과 결과를 뒤집는다」.
- **지배 판정을 `plan_id`별 예외 목록으로 두기(「이 안은 목적이 있으니 남긴다」).** 기각한다. 그것이 정확히 D52 후속의 읽기였고 D53이 깬 것이다. 이름이 목적을 보증하지 못하므로 **값으로 재고, 다른 값이 실제로 달라지면 남긴다.**
...
3. 버전 규약(**0.D절**)이 "기존 필드의 의미 변경"을 major로 정하고 있다. 계약이 보장하던 성질을 거두는 것은 그 범주다.
...
**(2) 열거형에서 값이 둘 빠진다.** `plan_id`의 `pension_contribution_limit_fill`과 `limited_by`의 `credit_limit`이다. 앞은 `options.plan_variants`로 보내면 **`unknown_plan_variant` 오류**가 되고, 뒤는 화면의 갈래를 죽은 코드로 만든다. 버전 규약(**0.D절**)이 "열거형에서 값 제거"를 major로 정한다.
...
1. **계약이 문서로 준 산식을 거둔다.** 5.5절이 `monthly_krw`를 `floor(annual_krw / months_remaining_in_tax_year)`라고 **적어 두었다.** 그 산식을 그대로 구현한 소비자는 계약을 어긴 적이 없는데 이제 다른 숫자를 낸다. 버전 규약(**0.D절**)의 「응답 쪽 보장을 거두는 것」이고 0.1절이 같은 근거로 major를 골랐다.
...
1. **규약이 그렇게 정한다.** 이 문서의 버전 규약(**0.D절**)이 major에 "**응답 쪽 보장을 거두는 것**"을 명시로 넣었고, 그 문장이 가리키는 사례가 0.1절이다. 지금이 정확히 그 형태다 — 요청 형태는 그대로이고 응답 값도 그대로인데 **계약이 준 범위 제약이 사라진다.**
...
### 0.19 왜 `11.0.0`(major)인가 — 금액은 1원인데 문장이 바뀐다 (D46 1번)

**요청은 한 글자도 바뀌지 않았고 새 필드도 없다.** 그런데도 major인 이유는 규약이 정한 두 범주에 동시에 걸리기 때문이다 — **같은 요청이 다른 금액을 내고**, **기존 필드의 뜻이 바뀐다.**

**(1) 같은 요청이 다른 금액을 낸다.** 「국고금 관리법」 제47조 제2항이 **국세의 과세표준액**에서만 1원 미만을 버리게 한다. 엔진은 근로소득공제를 먼저 버리고 총급여에서 뺐고, 그래서 **과세표준이 조문보다 항상 정확히 1원 컸다** — 우연이 아니라 항등식이며, 총급여가 20의 배수가 아닌 15% 구간(1,500만~4,500만) 사용자 **전원**에게 걸린다. 고치면 `tax_base_krw`가 1원 내려가고 `computed_tax_krw`·`cap_krw`가 따라 움직인다.

**(2) boolean이 뒤집히는 사용자가 실재한다 — 이것이 결정적이다.** 세액공제 상한이 정확히 1,350,000원(900만 × 15%)이라, 한도가 그 절단선의 **1원 안**에 놓인 사용자에게 `PlanTaxLiabilityCap.applied`가 `false` → `true`가 된다. 관리자가 검산한 좌표가 총급여 **34,143,911원**이고, 거기서 한도는 `1,350,000` → `1,349,999`, `applied`는 `false` → `true`다. **그 하나가 화면 문구를 통째로 바꾼다** — D37이 정한 「이 막대가 짧은 것은 덜 넣어서가 아니라 낼 세금이 이 한도보다 적기 때문입니다」가 나오느냐 마느냐다. **금액을 보는 어떤 검사도 1원을 잡지 못한다.**

**(3) `applied`의 뜻이 좁아졌다.** 이제 이 값은 **정확값끼리의 대소**가 정한다(룰셋 `tax.rounding.won_fraction`의 `comparison` 단계 — 「비교 전에 양쪽을 절사하지 않는다」). 그 결과 **`applied === (잘린 금액 > 0)`이 더 이상 성립하지 않는다.** 잘린 양이 1원에 못 미치면 `applied`가 `true`인데 표시 금액은 한 원도 줄지 않는다(총급여 24,795,208원 · 예산 2,666,667원이 그런 좌표다). 남는 보장은 한 방향이다 — **금액이 줄었으면 반드시 잘린 것이다.** 5.5절에 적었고, **화면이 「잘렸다」는 문장을 쓸 때는 `reduced_total_krw > 0`을 함께 보아야 한다.**

**(4) 조문이 우리 출력을 지목하지 않는 자리를 계약이 진다.** §47①(국고금의 수입·지출, **10원** 미만 절사)은 이 엔진의 출력에 **걸리지 않는다** — 우리가 내는 것은 납부세액도 환급세액도 아니라 세액공제로 줄어드는 세액과 그 한도다. 룰셋이 그 판정을 `binds_engine_output: false`로 값에 적었고, 엔진은 그 칸이 `false`가 아니면 10원 단위를 아무 데나 걸지 않고 **멈춘다.** 그래서 **응답 금액은 10의 배수가 아니다** — 그것을 가정한 소비자가 있다면 지금 깨져야 한다.

**중간값·비교·표시 세 단계는 조문이 정한 것이 아니다.** 룰셋이 `determined_by_law: false`로 표시했고, 계약도 그 구분을 그대로 진다 — `tax_base_krw`만 조문이 버린 값이고 나머지 정수 금액은 **우리 규약**이 표시 직전에 한 번 버린 값이다. 엔진 쪽에서도 두 자리가 다른 손잡이(`statutory()`/`convention()`)를 쓰며, 뒤바꿔 부르면 값이 나오지 않는다.

**채택하지 않은 대안 둘.**

- **patch로 처리하기**(「엔진이 조문을 잘못 구현한 것을 고쳤을 뿐 계약 문서는 그대로다」). **기각한다.** 소비자가 보는 값이 바뀌고 그중 하나는 boolean이며, 그 boolean에 화면 문구가 매여 있다. 0.1절이 같은 자리에서 patch를 이미 한 번 기각했다.
- **`applied`를 표시 금액의 뺄셈으로 정의해 (3)의 어긋남을 없애기.** **기각한다.** 그것은 절사한 값으로 비교하는 것이고, 룰셋의 `comparison` 단계가 명시로 금지한다 — 1원 미만의 차이가 사라지면 이번에 고친 결함이 다른 자리에서 그대로 되살아난다. **대신 어긋남을 값으로 드러내고 화면이 볼 곳을 지정했다.**

**목이 맞춰야 할 것:** 버전 문자열, 그리고 **`src/web/engine/mock-engine.js`의 세액 한도 산식** — 단계마다 버리고 있다면 같은 1원 차이가 목에 남는다. 이 유닛은 `src/web/`을 열지 않았다.
