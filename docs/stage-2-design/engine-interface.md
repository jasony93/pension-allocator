...
| `applied === (잘린 금액 > 0)`인가 | **아니다.** `applied`는 **정확값의 대소**이고 잘린 금액은 **표시 금액의 뺄셈**이다. 화면이 「잘렸습니다」를 쓸 때는 `reduced_total_krw > 0`을 함께 본다. 한 방향만 무조건 참이다 — **금액이 줄었으면 반드시 잘린 것이다** | `applied` · `reduced_*_krw` | 5.5절 · **D46 1번 · D49** |
