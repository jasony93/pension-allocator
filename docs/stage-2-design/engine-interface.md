...
open_questions:
  - "**[D54 집행·관리자에게 올린다] 정답지 두 자리가 값을 받아야 한다. 이 유닛은 정답지를 고치지 않았다.** `contribution_carryover_available`을 `applied`에서 떼어 **실제 잘림(표시 금액)**에 매달았고(계약 5.5절·0.22절), 골든 어휘 `tax_liability_cap.contribution_carryover_available`을 열었다. **채울 값은 22차 검증 리포트가 이미 산출해 두었다** — **GC-32d는 `false`**(정확값 초과분 0.1원 · 표시 초과분 0 · 대응 납입액 0.67원이라 인출을 의제할 수 없다), **GC-34는 `true`**(표시 초과분 1원 · 대응 납입액 6.67원이 연금저축 계좌에 남는다). 그 칸이 두 블록에 들어올 때까지 어휘는 `golden-cases.test.mjs`의 `VOCABULARY_DEBT`에 사유와 함께 있다. **값의 저자는 `tax-domain`이고 이 유닛은 `docs/stage-4-verification/`을 열지 않았다.**"
...
- **기준은 표시되는 세액공제액이다**(`13.0.0` · D53 2번). `12.0.0`은 정확값으로 재었고, 세액 한도에 소수부가 있으면 그 기준이 **0.1원짜리 증가**를 「낳았다」로 세어 15% 구간에서 1~6원, 12% 구간에서 1~8원의 IRP를 남겼다. 그 몫은 `tax_credit`에도 `tax_credit_before_cap`에도 나타나지 않으므로 **얻은 것이 아니고**, 그것 때문에 중도인출이 제한되는 계좌에 돈을 넣게 하는 것은 이 절이 방금 금지한 바로 그 형태다. 비교는 **소득세분과 지방소득세분의 표시 금액 합**으로 한다.
