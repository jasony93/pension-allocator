...
**32b가 정확히 같은 자리다.** 34,143,912원의 한도는 **1,350,000.076**이고, 버림·반올림·올림 어느 규약에서도 **1,350,000 이상**이라 자르지 않는다. 그러므로 이 좌표는 규약에 걸리지 않는다. **관리자가 D40에서 조문으로 검산한 좌표이고 내 산출도 같은 값을 냈다.** 계약 5.10절도 같은 사실을 적는다 — 그 총급여 위로는 (ISA 만기 전환 추가한도가 없는 한) 이 한도가 아무것도 자르지 않는다.
...
| **GC-32b** | 1,350,000.076 | 9,000,000.5 → 한도를 다 못 쓴다 | 9,000,000 (인정액 상한이 먼저 걸린다) | 6,000,000 | **3,000,000** | 0 |
