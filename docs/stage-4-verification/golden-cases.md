...
inputs:
  ...
  - docs/org/charter.md
open_questions:
  - "**[25차·정답지가 틀린 것 1건 — GC-P1b에 지방소득세를 두 번 얹었다]** 처음 적은 「**580,800원**(소득세 528,000 + 지방 52,800)」이 틀렸고 **528,000원**(소득세 480,000 + 지방 48,000)이 맞다. **528,000은 이미 `480,000 × 1.1`인데 그것을 소득세분으로 놓고 한 번 더 얹었다** — 잘못은 산술이 아니라 **단위**이고, 룰셋 머리말 `notes.local_income_tax`의 경고와 반대편의 같은 함정이다. **`calc-engine-dev`가 잡았고 엔진이 옳았다**(시험이 룰셋의 곱 480,000 + 48,000을 걸고 통과 중이었다). **그리고 같은 문서 18.1절의 차액 검산(`660,000 − 132,000 = 528,000`)이 이미 그 오류를 잡고 있었는데 맞춰 보지 않았다** — 재산출하면서 그 검산을 18.2절에 상설로 붙였다. **정답지가 커질수록 문서 안의 교차 검산을 실제로 밟는 절차가 필요하다.** 18.2절, `tax-rules-report.md` 31.9절."
...
|---|---|
| 프로필 | GC-P1과 같되 만 **70세** |
| **기대 결과** | **528,000원.** 소득세 480,000 + 지방 48,000 |

**도출 과정.** `pension.income.withholding_rate.by_age`의 둘째 구간이 「70세 이상 80세 미만」이므로 **만 70세 생일 당일부터 4%**다. 경계가 아래 구간이 아니라 **위 구간**에 붙는다 — `pension.income.withholding_rate.deferred_retirement`의 경계가 반대 방향(10년 **이하**가 70%)인 것과 짝을 이룬다.

1. `pension.income.withholding_rate.by_age` 둘째 구간 → **4%**. `12,000,000 × 0.04 = 480,000`
2. `tax.local.personal_income_surtax` → `480,000 × 0.1 = 48,000`
3. 합계 **528,000원**

**GC-P1과의 차이로 검산한다.** 두 케이스는 세율만 1%p 다르므로 차액은 `12,000,000 × 0.01 = 120,000`(소득세)에 지방소득세를 얹은 **132,000원**이어야 한다. `660,000 − 132,000 = 528,000` — 맞는다.

> **[25차 정정] 이 케이스를 처음 적을 때 「580,800원(소득세 528,000 + 지방 52,800)」으로 냈고, 그것은 틀렸다.** 528,000은 **이미 지방소득세를 포함한 합계**인데 그 값을 소득세분으로 놓고 지방소득세를 **한 번 더** 얹었다. 잘못은 산술이 아니라 **단위**에 있었다 — 파일 머리말 `notes.local_income_tax`가 「실효율을 상수로 박지 말고 두 규칙을 곱해 산출한다」고 경고한 그 자리이고, 나는 곱한 결과를 곱하기 전의 값으로 되읽었다. **엔진 시험이 룰셋의 곱(480,000 + 48,000)을 이미 걸고 통과 중이었으므로 엔진이 옳고 정답지가 틀렸다.** 같은 문서 18.1절의 차액 검산(`660,000 − 132,000`)과도 어긋나 있었는데, **한 문서 안에 교차 검산이 있었는데도 내가 맞춰 보지 않았다.** 이 케이스에 위 검산 문단을 상설로 붙인 이유가 그것이다.
