...
open_questions:
  ...
  - "**[15차·세법 사안이 아닌 지적 하나]** 계약 0.12절이 `monthly_annualized_krw`와 `annual_krw`의 차이 범위를 `±(개월수 − 1)`로 적었는데 **산술로 성립하지 않는 구석이 있다** — 이탈은 `조정액 × 개월수 − (연 배분 mod 개월수)`이고 조정액 상한이 3이므로 조정액 2·`m=12`·`mod=11`이면 **13**이다. 한 갈래가 조정액을 둘 이상 받으려면 다른 갈래가 전부 막혀야 해서 **도달 가능한지는 확인하지 못했다**(엔진을 읽어야 한다). **한도 위반으로는 이어지지 않는다** — 조문이 거는 상한은 규칙 3이 따로 지킨다. `calc-engine-dev`·관리자 서술 정정 사안이다."
...
**세법이 정하지 않는 것을 하나 더 적어 둔다.** 1원째가 **연금저축**(인출 제약이 있는 계좌)으로 간 것은 「벗어남이 가장 작은 갈래」라는 계약의 기준이 그렇게 정한 결과이지, 인출 자유 계좌를 먼저 채우라는 배분 순서 원칙을 따른 것이 아니다. 계약이 그 사실을 스스로 적었고(0.12절), 이 유닛은 **크기를 근거로 다투지 않는다** — 갈래당 연 (개월수 − 1)원 이하, 이 케이스에서는 1원과 2원이다.
