...
> **16차 정정 — 위 한 줄은 6차 시점의 값이고 D32 이후 낡았다.** 지금 이 케이스의 배분은 **연금저축 7,000,000** + IRP 3,000,000 + ISA 20,000,000이고 미배분은 **0**이다(14.1절 표). 잘린 금액과 세액에 관한 이 절의 결론은 그대로다 — 3차 몫 1,000,000은 공제를 낳지 않으므로 자르기 전 금액도 자른 뒤 금액도 움직이지 않는다. **바로 그 사실이 아래 `non_quantified_effects`가 주장하는 것이다.**
