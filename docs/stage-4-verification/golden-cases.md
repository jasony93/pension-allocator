...
**추징 경고에 대한 판단.** `isa.early_termination.clawback`은 "계약기간이 **3년이 되는 날 전에** 계약을 해지하는 경우"에만 걸린다(조특법 §91조의18⑦). 경과 3년이면 그 요건이 성립할 수 없으므로 **추징을 근거로 한 경고는 세법상 성립하지 않는다.** 다만 계약 8.4절이 경고 조건을 horizon 값만으로 정하고 있어, 고칠 곳이 엔진인지 계약인지는 관리자 판정이다(머리말 `open_questions`).
