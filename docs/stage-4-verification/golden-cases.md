...
| 30,686,275 | 19,333,333.75 | 1,640,000.06 | 740,000 (§59② 한도) | **900,000** | 아니오(상한) |
| 34,142,000 | 22,270,700 | 2,080,605 | 730,864 (§59② 한도) | **1,349,741** | 아니오(상한) |
| 34,143,912 | 22,272,325.2 | 2,080,848.78 | 730,848.70 | **1,350,000** | 아니오(상한) |
...
| 55,000,001 | 41,000,000.95 | 4,890,000.14 | 660,000 | **4,230,000** | 아니오(상한) |
...
**[20차 정정 — 조문이 나왔다. 이 문단의 규약은 조문이 아니다.]** 「국고금 관리법」 제47조 제2항이 **국세의 과세표준액을 산정할 때 1원 미만의 끝수를 계산하지 아니한다**고 정한다(제1항은 국고금의 수입·지출에서 10원 미만, 제3항은 지방자치단체에 준용할 **수 있다**). **버림이고, 조문이 지목한 단계는 둘뿐이며, 그 둘 중 어느 것도 이 문서가 곤란했던 중간 단계가 아니다.** 룰셋 `tax.rounding.won_fraction`이 정의 자리이고, 거기 세운 규약은 **「§47②는 과세표준에 적용하고, 나머지 중간 단계는 절사하지 않으며, 비교는 정확값으로 하고, 표시 직전에 한 번 버린다」**이다. 이 문단의 「마지막에 원 미만을 버린다」는 그중 마지막 단계만 맞고 **§47②를 적용하지 않는다** — 위 34,143,911원에서는 두 규약의 표시 금액이 우연히 같지만(둘 다 1,349,999) 언제나 같지는 않다. **엔진은 세 번째 규약을 쓰고 있고 그것이 조문과 어긋난다**(`verification-report.md` 20절). **좌표를 피한 상태는 그대로 두었다** — 넣으면 즉시 빨간색이 되고, 초록으로 만드는 방법이 엔진 수정과 짝이라 관리자 판정 뒤에 한다.
...
### GC-32a~c - 한도가 공제액보다 적을 때 / 정확히 같을 때 / 많을 때 【경계 3건 · 6차 추가 · 17차 좌표 이전】
...
**세 좌표를 이렇게 골랐다 — 반올림 규약이 결과를 가르는 자리를 피한다.**

| | 해당연도 총급여 | 산출세액 | 근로소득세액공제 | 한도 | 인정 소득세분 | 지방세분 | 합계 | 잘렸는가 |
|---|---|---|---|---|---|---|---|---|
| **GC-32a** | **34,142,000** | 2,080,605 | 730,864 | **1,349,741** | **1,349,741** | **134,974** | **1,484,715** | **예** (259원) |
...
**32a의 총급여가 왜 저 수인가.** 절단선 바로 아래인 34,143,911원을 쓰면 한도가 **1,349,999.9405**가 되어 **버림이면 자르고 반올림이면 자르지 않는다.** 원 미만 처리 규약을 이 문서가 정할 근거가 없으므로 **그 규약에 결과가 걸리는 좌표를 피한다.** §59②의 감액률이 8/1000이라 총급여액이 **2,000의 배수**이면 산식 전체가 정수로 떨어지고, 그런 수 중 절단선 바로 아래가 34,142,000원이다. **1원 차이가 259원 차이로 넓어졌지만, 검사하는 사실(`<`인가 `≤`인가)은 한 글자도 달라지지 않는다.**
...
**도출 과정** 세 건 모두 S1~S13이 GC-31과 같아 자르기 전 소득세분이 1,350,000이다(공제율도 셋 다 15% 구간이다). S14의 인정액 = `min(1,350,000, 한도)`.

- **32a** — `min(1,350,000, 1,349,741) = 1,349,741`. 지방세분은 **인정된** 소득세분에 부가율을 곱한다(`local_tax_follows_income_tax_cap`): 1,349,741 × 0.1 = 134,974.1 → **원 미만 버림으로 134,974**. 지방세분을 자르기 전 값 135,000으로 두면 인정되지 않은 공제에 붙은 지방세가 남는다.
...
**왜 이 수인가.** 관리자가 D40에서 조문으로 검산한 세 좌표 중 하나이고, **한도가 정확히 900,000원**이 되는 자리다. 내 산출도 같은 값을 냈다 — 근로소득공제 9,852,941.25(§47) → 근로소득금액 20,833,333.75 → 과세표준 19,333,333.75(§50①1) → 산출세액 1,640,000.06(§55①) → 근로소득세액공제 **740,000**(§59② 총급여 3,300만원 이하의 74만원 한도가 §59①의 817,000을 자른다) → 한도 **900,000.06**. 원 미만을 버려 900,000원이고, **반올림해도 같은 값이라 규약에 걸리지 않는다.**

**기대 결과** 자르기 전 소득세분 1,350,000이 한도 900,000에 걸려 **900,000 / 90,000 / 990,000**으로 잘린다. 안내 `tax_liability_cap_applied` · `tax_liability_cap_estimated_from_total_salary`.
